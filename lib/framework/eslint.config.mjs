...
export default [
  ...
  // module importing a sibling is fine.
  {
    files: [
      ...
      // the tier order with nothing to flag it. Naming it here restores `leafBan`.
      //
      // TWO SEED DIRECTORIES, AND ONLY ONE IS FRAMEWORK TIER:
      //
      //   `prisma/seeds/framework/**`  — framework-tier seeds (data and config
      //     Daybreak owns, e.g. the rubric judge). Framework code: `leafBan` applies.
      //
      //   `prisma/seeds/_framework/**` — deliberately ABSENT. It holds exactly one
      //     unit, `000-framework-boot.ts`, and that unit is a BRIDGE: its whole job
      //     is to compose the two tiers, calling `@/lib/framework`'s boot sequence
      //     with the leaf's `initLeafApp` passed in. It is the seed-time twin of
      //     `lib/app/bootstrap.ts`, and like that file it has to see both sides.
      //     `leafBan` would forbid the one import it exists to make.
      //
      //     That is why the directory is named for ordering (`_` sorts between
      //     digits and letters) and holds nothing else: a genuine framework seed
      //     dropped in here would inherit a bridge's exemption it has no claim to.
      //     Put those in `framework/`.
