...
export default [
  ...
  {
    files: ['**/*.{ts,tsx}'],
    ignores: ['lib/framework/**', 'lib/app/**', 'tests/**/lib/framework/**'],
    rules: {
  ...
  // ── Framework tier ───────────────────────────────────────────────────────
  // `lib/framework/**` + its reserved admin surfaces (empty until later
  // features, governed as framework tier from day one) + the framework's own
  // tests. Framework → core is allowed; framework → leaf (`@/lib/app/**`) is
  // not (it would invert the tier order). The `@/lib/framework` self-ban is
  // deliberately dropped here — a framework module importing a sibling is fine.
  {
    files: [
