...
export interface ModuleAgentBindingView extends ModuleAgentBinding {
  agent: { id: string; name: string; slug: string; isActive: boolean } | null;
}

/**
 * List a module's agent bindings, primary seat first then by role, each stitched
 * with the bound agent's display fields. A soft-deleted agent still resolves (with
 * `isActive: false`) so a stale binding is visible; a hard-deleted agent's binding
 * is already gone via the FK cascade. Unknown module ⇒ 404 (not an empty list).
 */
