...
| `lib/app/ci.ts`                            | coverage exclusions / always-run tests / ownerless-surface exceptions               | `vitest.config.ts` + `scripts/ci/scoped-tests.ts` + `lib/orchestration/access/ownerless-surfaces.ts` (CI / `/pre-pr`)              |
| `lib/app/leaf-ci.ts`                       | coverage exclusions / always-run tests / ownerless-surface exceptions (your leaf's) | `vitest.config.ts` + `scripts/ci/scoped-tests.ts` + `lib/orchestration/access/ownerless-surfaces.ts` (CI / `/pre-pr`), via `ci.ts` |
...
distinct from the existing sections. To render your own brand lockup as the
section header instead of the default uppercase label, pass `titleNode` (any
`ReactNode`); `title` stays required and remains the React key, the registry's
dedupe key, and the heading's accessible name, so a wordmark image can't cost you
the label. (Daybreak fills `lib/app/admin-nav.ts` to register its own "Framework"
section and then calls `initLeafAdminNav()`, so you edit the reserved
`leaf-admin-nav.ts` seam it delegates to — never `admin-nav.ts` itself, which
would conflict on a Daybreak upgrade.)
