...
| `guard`             | `guard-editor.tsx`          | `rules` (Textarea, hidden in schema mode), `mode` (Select: `llm` / `regex` / `schema`), `failAction` (Select: `block` / `flag`), `maxRetries` (number 0–10, shown only when `failAction` is `block`), `modelOverride` (Input, LLM mode only), `temperature` (number, LLM mode only), `schemaName` (Input, schema mode only — registry slug from `lib/orchestration/schemas/registry.ts`), `inputStepId` (Input, schema mode only — step whose output is validated; blank = workflow input) |
...
- **`workflowDefinitionToFlow(def)`** — turns each `WorkflowStep` into a `PatternNode`. Position: if `step.config._layout = { x, y }` exists, use it verbatim; otherwise run a simple BFS levelling from `entryStepId` (`x = level * 220`, `y = idxInLevel * 150` with a 40px stagger on odd levels). One edge per `ConditionalEdge`; `condition` string becomes the edge label. Edges with `maxRetries > 0` get `type: 'retry'` and an appended label suffix `(retry ×N)`. If a `ConditionalEdge` carries `_layout: { controlPointX, controlPointY }`, that point is surfaced on `edge.data.controlPoint` so the custom retry edge component can render the bow exactly as the user last dragged it.
- **`flowToWorkflowDefinition(nodes, edges, opts)`** — inverse. Stashes each node's `x/y` back into `step.config._layout` on save. Strips any pre-existing `_layout` first so layouts don't nest on re-save. Derives `entryStepId` as the first node with no incoming edge (fallback: first node in the list). Strips the `(retry ×N)` label suffix before writing back to `ConditionalEdge.condition`. Reads `maxRetries` from edge data; for guard steps with `failAction: 'block'`, also applies `config.maxRetries` to fail-condition edges. Reads `data.controlPoint` from each edge and writes it back to `ConditionalEdge._layout` so a dragged retry-edge shape persists across save/reload.

**Custom edge types.** The canvas registers custom edge components via `edgeTypes` in `workflow-canvas.tsx`. Currently one custom type:

- **`retry`** (`edge-types/retry-edge.tsx`) — dashed amber line (`#f59e0b`, `strokeDasharray: 6 3`) drawn as a quadratic bezier through a control point. The label is a draggable amber pill (with a `Move` icon and the condition + retry count, e.g. "fail (retry ×2)") that doubles as the edge-shape control: mouse-down begins a drag; document-level mousemove computes a new control point such that the label tracks the cursor 1:1 (`cp = 2 * cursor - midpoint`). The control point persists on `edge.data.controlPoint` and round-trips through the workflow mappers. When no control point is set, the component computes a perpendicular default (~150px) so the edge has a sensible bow on first render. The pill renders with `zIndex: 1500` so it stays grabbable when the curve passes over a node. Used for bounded retry back-edges where `ConditionalEdge.maxRetries > 0`. All other edges use the React Flow `default` type.

**Guard retry config.** When a guard step's `failAction` is `block`, the guard editor shows a "Retry on failure" number input (0–10). Setting this to > 0 creates a bounded retry back-edge: the guard's Fail output, when wired to an upstream step, will retry that step up to `maxRetries` times. The failure context is forwarded as `ctx.variables.__retryContext` and the retry target's prompt can read it via `{{vars.__retryContext.failureReason}}` (and conditional blocks like `{{#if vars.__retryContext}}...{{/if}}` so the preamble only appears on retry attempts). See [engine.md → Bounded retry loops](../orchestration/engine.md#bounded-retry-loops).
