...
| `guard`             | `guard-editor.tsx`          | `rules` (Textarea), `mode` (Select: `llm` / `regex`), `failAction` (Select: `block` / `flag`), `maxRetries` (number 0–10, shown only when `failAction` is `block`), `modelOverride` (Input, LLM mode only), `temperature` (number, LLM mode only)                                                                                           |
