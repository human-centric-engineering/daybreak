...
epic: Framework v1
status: shipped (t-1 #73 · t-2 #74 · t-3 #75 · t-4 escalation delivered under f-emergence (18) t-1 #78) — all four kinds shipped
owner: John
...
| t-4 | **Escalation-pathway kind.** The `escalation` payload (`{signal, scope?, response:{routeToRole?, notify?, workflowSlug?, resources?}}`); the resolver that, on a detected signal in scope, dispatches the workflow (via the shipped `drainEngine` bridge), notifies (`notifyEscalation`), and **always logs** (audit; + `JourneyEvent` when user-scoped).                                                                                                                  | `lib/framework/facilitation/policies/escalation.ts`, `lib/framework/facilitation/policies/kinds.ts` (+ kind), `tests/…`                                                                                                                                   | t-1  | ✅ done (f-emergence t-1) | #78 |

**Sizing (B1): 4 promoted PRs.** The board's four indicative tasks hold — re-cut along the
...
isolated), **t-4** composes the guard-event / notify / audit bridges. t-2/t-3/t-4 all depend on t-1
and are mutually independent.

**Close-out (2026-07-07):** all four kinds shipped. t-1 (#73), t-2 (#74), t-3 (#75) landed here; **t-4
(escalation, F15)** was first **deferred** at this feature's close-out, then **picked up and delivered
under `f-emergence` (18) t-1 (#78)** — its natural home (escalation needed a second fork-carried core
seam, a post-detection guard-event observer, which f-emergence owns). The escalation kind's code
