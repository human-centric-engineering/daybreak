...
epic: Framework v1
status: shipped (t-1 #93 · t-2 #95 · t-3a #96 · t-3b #98)
owner: John
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---- | -------- | --------- |
| t-1 | **Node-embedding store + sync + on-demand trigger + drift bridge (anchor).** `FrameworkNodeEmbedding` model (`vector(1536)` + hand-appended HNSW) + `--create-only` migration; the node-text composer (extract nodes from the published version, compose `key` + module name/desc + meta); the embed+upsert sync service (`embedBatch`); the `POST …/maps/[slug]/embeddings` admin trigger; the drift bridge (decision 4).                                                                                         | `prisma/schema/framework-facilitation.prisma`, `prisma/migrations/…`, `lib/framework/facilitation/overlays/{node-text,embed-sync,queries}.ts`, `lib/framework/db-drift.ts`, `lib/app/db-drift.ts`, `lib/app/leaf-db-drift.ts`, `app/api/v1/admin/framework/maps/[slug]/embeddings/route.ts`, `tests/…` | —    | **done** | #93       |
| t-2 | **Similarity → the `related` advisory slot in guidance (F9-safe).** A `findRelatedNodes(graphSlug, nodeKey, k)` cosine query + wire it into `loadGuidance` to fill `RankedMove.related` downstream of `rankMoves`, labelled advisory, never feeding availability; graceful-empty when no embeddings. (No dimension drift-guard needed — similarity is node-to-node over vectors from the _same_ sync run, so there's no fresh-query-vs-stored comparison to guard.)                                                | `lib/framework/facilitation/overlays/{queries,related}.ts`, `lib/framework/guidance/guidance.ts`, `lib/framework/facilitation/map/version-service.ts`, `tests/…`                                                                                                                                       | t-1  | **done** | #95       |
| t-3 | **Proactive guidance: scheduled sweep + nudge (F13).** A system-scoped active-journeys enumerator + `runProactiveGuidanceSweep()` (per journey: `loadGuidance` under a self-viewer → stall/next-step signal) + the `framework_journey_nudge` throttle table (+ migration) + email nudge + `registerStepType('framework_proactive_guidance')` + an on-demand admin trigger. **Split at build into t-3a (sweep + preview, #96) and t-3b (throttle table + email nudge + step type, #98)** per the sizing note below. | `prisma/schema/framework-facilitation.prisma`, `prisma/migrations/…`, `lib/framework/facilitation/overlays/{active-journeys,proactive-sweep,nudge,proactive-step}.ts`, `emails/proactive-nudge.tsx`, `app/api/v1/admin/framework/facilitation/proactive-guidance/…/route.ts`, `tests/…`                | t-1  | **done** | #96 · #98 |
