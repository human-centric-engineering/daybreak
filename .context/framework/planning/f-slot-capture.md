...
epic: Framework v1
status: in flight (t-1/t-2 shipped · t-3 in flight · t-3b/t-4 backlog)
owner: John
...
| t-3  | **Sensitivity masking + local typed-value handling.** `slotMaskingPolicy` (special_category strictest) applied before storage; `SLOT_DATA_TYPE → typed-value` bridge (`typedValueSchema` map + `validateTypedValue` local check) — agent-supplied `valueJson` validated locally, no LLM. The `#307` prose→typed **extraction fallback** is split off to **t-3b** (needs agent→provider→model resolution `fill_slot` doesn't receive + the `runStructuredCompletion` relocation ask). | `data-slots/capabilities/{masking,typed-value}.ts`, `tests/…`                                                                         | t-2  | in flight (John) | —   |
| t-3b | **#307 prose→typed extraction fallback (deferred from t-3).** Secondary `runStructuredCompletion` (using `typedValueSchema` as the enforced `responseSchema`) that runs ONLY when the typed form is absent/invalid or the source is prose. Needs the agent's provider/model resolved into the capability context + the [[upstream-asks]] `runStructuredCompletion` relocation.                                                                                                       | `data-slots/capabilities/{typed-value,extract}.ts`, capability-context provider seam, `tests/…`                                       | t-3  | backlog          | —   |
