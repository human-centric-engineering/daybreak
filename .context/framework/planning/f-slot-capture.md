...
epic: Framework v1
status: shipped (t-1 #42 · t-2 #43 · t-3 #44 · t-3b #45 · t-4 #46)
owner: John
...
7. **`fill_slot` #307-enforced typed extraction — net-new mapping (t-3).** The
   ...
   **Resolved (location):** keep `runStructuredCompletion` in
   ...
   > **Delivered split (t-3 → t-3b).** t-3 ships the shared `typedValueSchema`/`validateTypedValue`
   > map **and the local-validate path** (agent-supplied `valueJson`, no LLM). The **prose→typed
   > extraction fallback** (`runStructuredCompletion`) is deferred to **t-3b**: it needs the
   > agent's provider/model resolved into the `CapabilityContext` (which `fill_slot` does not
   > receive today) and the [[upstream-asks]] relocation above, and its `phase` union
   > (`'summary' | 'scoring'`) doesn't fit slot capture. The map is built now so t-3b is a thin
   > add. Result: the common typed path is complete and silent; only the rarer prose→typed
   > conversion waits on t-3b.
...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- | ---- | ------ | --- |
| t-1  | **Framework capability-registration seam + `get_state`.** The generic non-module framework-capability register + `ai_capability` sync wired into `syncFramework()`, proven by `get_state` (`canRead` → `getSlotHeads`, `processesPii`, silent).                                                                                                                                                                                                                                      | `lib/framework/data-slots/capabilities/{get-state,index}.ts`, framework-capability register/sync, `lib/framework/index.ts`, `tests/…` | —    | done   | #42 |
| t-2  | **`fill_slot` (the write cap).** Targeted-slug validation / open-mode minting / P2002 retry / `appendSlotValue`; `processesPii` + `redactProvenance`; silent (D5).                                                                                                                                                                                                                                                                                                                   | `lib/framework/data-slots/capabilities/fill-slot.ts`, `tests/…`                                                                       | t-1  | done   | #43 |
| t-3  | **Sensitivity masking + local typed-value handling.** `slotMaskingPolicy` (special_category strictest) applied before storage; `SLOT_DATA_TYPE → typed-value` bridge (`typedValueSchema` map + `validateTypedValue` local check) — agent-supplied `valueJson` validated locally, no LLM. The `#307` prose→typed **extraction fallback** is split off to **t-3b** (needs agent→provider→model resolution `fill_slot` doesn't receive + the `runStructuredCompletion` relocation ask). | `data-slots/capabilities/{masking,typed-value}.ts`, `tests/…`                                                                         | t-2  | done   | #44 |
| t-3b | **#307 prose→typed extraction fallback (deferred from t-3).** Secondary `runStructuredCompletion` (`typedValueSchema` object-wrapped as the enforced `responseSchema`) that runs ONLY when a typed slot gets no valid `valueJson`. Resolves the capturing agent's own provider/model (`resolveAgentProviderAndModel` off `context.agentId`); best-effort — a provider/parse/timeout failure never fails the capture.                                                                 | `data-slots/capabilities/extract.ts`, `fill-slot.ts`, `tests/…`                                                                       | t-3  | done   | #45 |
| t-4  | **Per-agent read/write exposure via grant `customConfig`.** Each capability re-reads its own `AiAgentCapability.customConfig` (Zod-parsed into `{read?,write?:{groups?,scopes?}}`) and enforces it against the slot's `group`/`scope`. Tri-state: absent = permissive, valid = enforced, malformed = fail-closed. `get_state` filters heads (definition join only when an allowlist is set); `fill_slot` refuses out-of-scope writes + mints.                                        | `data-slots/capabilities/{exposure,get-state,fill-slot}.ts`, `queries.ts`, `tests/…`                                                  | t-2  | done   | #46 |
