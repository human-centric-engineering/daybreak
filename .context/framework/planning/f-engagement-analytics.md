...
status: shipped
owner: John
shipped: 2026-07-10 — t-1a #140 · t-1b #141 · t-2 #142 · t-3 #143 (closes the Framework v1.1 follow-on epic)
...
- **Shipped as #143.** Two definitive emit sites, `session.started` dropped (above). `module.completed`
  wired into `applyJourneyTransition` (`guidance/guidance.ts`) after a committed `complete` — the pure
  engine `applyEvent` untouched (F11); completion read from the `node_completed` stream (repeatable-safe),
  idempotent per (user, module). `module.status_changed` fired from `updateModuleSettings` via
  `runModuleWorkflowBindings` in a new `MODULE_LIFECYCLE_EVENT_TYPE` map (no `JourneyEvent`, isolated
  `.catch`). New unit test for the checker + integration assertions for the dispatch and the transition-caller
  wiring (fires only on a committed complete of a module node). No new table, no migration, no core edit.
