...
status: shipped
owner: John
shipped: 2026-07-09 — t-1 #131 (+ #132 review-fix) · t-2 #134 · t-3 #136 · t-4 #137
...
| t-2 | **`submit_proposal` authoring capability** — a framework capability that writes a pending proposal, granted to the `facilitator` seat                                                                  | `lib/framework/facilitation/emergence/capabilities/{submit-proposal,index}.ts`, registration in `lib/framework/index.ts`, `tests/…`                                                                                                                              | (soft: t-1 for non-map subjects) | **done** #134         | —   |
| t-3 | **Scheduled eval sweep + rubric judge** — a `framework_eval_sweep` step type looping the shipped scorers over recent un-scored conversations + a seeded framework rubric judge + `rubricScore` storage | `lib/framework/facilitation/evaluation/{sweep-step,recent-conversations,rubric}.ts`, `framework-facilitation.prisma` (+`rubricScore`/`rubricReasoning`) + migration, `prisma/seeds/framework/001-framework-rubric-judge.ts`, `lib/framework/index.ts`, `tests/…` | —                                | **done** #136         | —   |
| t-4 | **Auto-embed-on-publish + webhook nudge channel** — fire-and-forget embed after the 4 publish paths (via a `map/publish-hooks` seam) + an env-gated outbound-webhook nudge alongside email             | `lib/framework/facilitation/map/{version-service,publish-hooks}.ts`, `lib/framework/facilitation/overlays/{embed-sync,nudge,nudge-channel}.ts`, `.env.example`, `lib/framework/index.ts`, `tests/…`                                                              | —                                | **done** #137         | —   |
...
- **Shipped as #136.** Two `/code-review` corrections ([[planning-retro#B31]]): **(a)** the "un-scored" watermark is **conversation-level "never scored at all"**, not per-turn — supervise writes only one anchor row and unpaired assistant turns never get rows, so a per-turn watermark re-scored (re-paid) those conversations every tick. **(b)** the rubric reasoning lives in its **own `rubricReasoning` column**, not merged into the metric scorer's `judgeReasoning` (which would clobber it / race). Migration adds `rubricScore` + `rubricReasoning`.
...
- **Shipped as #137.** The auto-embed fires through a **framework-local `map/publish-hooks` seam** (`registerMapPublishListener` / `notifyMapPublished`), NOT a direct `version-service → overlays` call — `/code-review` caught that the direct call inverted the map/overlays layering into a cycle ([[planning-retro#B31]]). The fork-owned env is read by a framework-tier `overlays/nudge-channel.ts` (Zod-validated), not the leaf-reserved `lib/app/env.ts`. A shared `postJsonWebhook` util is a **candidate upstream ask** (the escalation-notifier precedent is Sunrise-core) — [[upstream-asks]].
