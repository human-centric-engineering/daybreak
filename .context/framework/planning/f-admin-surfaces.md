...
| t-4 | **Ops-views UX polish** — searchable `?q=` roster pickers (typeahead) across the 3 binding tabs + per-binding `config`-override editing on the agents tab | `components/admin/framework/module-detail/{use-binding-roster,agents-tab,workflows-tab,knowledge-tab}.tsx` (+ new `roster-search.tsx`), `tests/…`                                                                                                                         | —    | **done** | #127 |
