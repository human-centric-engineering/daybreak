...
6. **Deep-link to the real editors; degrade honestly where none exists (map editor is feature 14, not
   built).** Confirmed targets: **agent** → `/admin/orchestration/agents/[id]`, **workflow** →
   `/admin/orchestration/workflows/[id]`, **module** → `/admin/framework/modules/[slug]` (the tabbed
   detail from [[f-ops-views]]). **No target yet:** the **map editor** (`f-map-editor`, 14 — unbuilt) and
   a **dedicated slot editor** (none). Map / slot / facilitation nodes get a graceful non-link state (a
   "no editor yet" affordance, not a dead link); when 14 lands, wiring the map deep-link is a one-line
   add. (Deep-links are a thin layer — folded into t-2 per [[planning-retro#B1|B1]], not their own PR.)
...
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------- |
| t-1 | **The composition-graph read endpoint (anchor).** All-modules aggregate readers (one query per pivot across all modules + one batched core-row stitch each, reusing the canonical batch-stitch) + the facilitation-layer assembler + the published-map projection, composed into a typed `CompositionProjection` (normalized entities + edges, ISO-string dates); `GET /api/v1/admin/framework/atlas` (`withAdminAuth`). Pure server, unit-testable with a prisma mock; assert the projection shape **and** no-N+1 (one query per entity type).                                                                                                                                                                                                    | `lib/framework/atlas/{queries,assemble,view}.ts`, `app/api/v1/admin/framework/atlas/route.ts`, `tests/…`                   | —                                                                                                                                                                    | **todo** | —        |
| t-2 | **The read-only composition canvas + deep-links (semantic zoom base).** The `@xyflow` multi-type node canvas (reusing the journey-explorer read-only config) + the pure-TS composition mapper (layout: map regions/places as containers, modules + the facilitation node with satellite clusters) + the node-type set + base **semantic zoom** (unfold satellites past a zoom threshold via `useStore`/`useOnViewportChange`) + **click-through deep-links** to the agent/workflow/module editors with a **graceful non-link state** for map/slot nodes (14 unbuilt). Nav "Atlas" item + thin server page. **May split at build** (t-2a static render + deep-links + node types · t-2b semantic-zoom + region containers) per [[planning-retro#B25 | B25]].                                                                                                                     | `components/admin/framework/atlas/{atlas-view,atlas-canvas,atlas-mapper,nodes/*}.tsx`, `app/admin/framework/atlas/page.tsx`, `lib/framework/admin-nav.ts`, `tests/…` | t-1      | **todo** | —   |
| t-3 | **Cross-cutting lenses.** The agent / slot-group / workflow-centric inversions as a client-side filter/highlight over the one projection (select an entity → focus every place it participates; dim the rest), a lens selector, and the focus/clear interaction.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `components/admin/framework/atlas/{lenses,atlas-view}.tsx`, `lib/framework/atlas/lens.ts` (pure filter helpers), `tests/…` | t-2                                                                                                                                                                  | **todo** | —        |
...
- **Map-node deep-link** — wire to the map editor when `f-map-editor` (14) ships (open question 3).
