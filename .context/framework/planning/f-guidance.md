...
epic: Framework v1
status: shipped (t-1 #49 · t-2 #51 · t-3 #52 · t-4 #57 · t-4b #61 · t-5 #59)
owner: John
...
5. **Context injection enriches the existing `'module'` contributor — single composed block per
   ...
   **user-agnostic** half — `loadModuleContext` composes the module's **name + description** from the
   ...
   "the module's config-relevant context". The **per-user fresh-slots injection** is split to **t-4b**,
   which unblocked it with a **fork-carried core-seam widening** (the #385/#403 pattern used by
   f-module-bindings t-4): `buildContext`/`ContextContributor` now pass a generic `ContextRequest {
userId? }` and cache per `(type, id, userId)` — a minimal, framework-agnostic core edit (empty
   `userId` = prior behaviour; boundary CI green), ledgered in [[upstream-asks]]. t-4b then injects the
   user's fresh slot values for the module's open declared slots. _(Full journey-position-in-prompt
   needs a module→graph→journey resolve; still deferred — agents get it per turn via the t-2
   capabilities.)_ The boundary test (strip framework → `buildContext` has one fewer contributor) is
   proven by `boot.test.ts`.
