...
| Lead                    | Simon Holmes                                                                                                                                                                                                                                                                                                                                                                                               |
| Status                  | `in flight` — `f-seams` + `f-bootstrap` + `f-module-core` + `f-map` + `f-slots` + `f-journey-state` + `f-engine` + `f-slot-capture` + `f-module-bindings` + `f-module-config` + `f-guidance` + `f-facilitation-agents` + `f-policies` **shipped**; `f-ops-views` (Simon) + `f-emergence` (John) **in flight**; **4 features available to claim** (`f-engagement`, `f-map-editor`, `f-overlays`, `f-atlas`) |
...
| 16  | `f-atlas`               | _unclaimed_     | **available** ▲                                 | f-module-bindings ✅, f-facilitation-agents ✅, f-slots ✅   | 3    | Framework atlas (composition view)                 |
| 17  | `f-policies`            | John            | **shipped** (#73 / #74 / #75)                   | f-facilitation-agents ✅                                     | 3    | Typed facilitation policy kinds                    |
| 18  | `f-emergence`           | John            | **in flight**                                   | f-engine ✅, f-facilitation-agents ✅                        | 5    | Proposal pipeline + escalation (F15) + eval wiring |
...
**Claimable right now (▲) — four open features.** **`f-policies` (17)** is now **shipped** (#73 / #74 / #75; detailed plan: [[f-policies]]) — the typed governance-policy kinds under one `FacilitationPolicy` table (a `kind` discriminator + per-kind Zod payload, F14): the spine + **auto-approval** stored knob (t-1), **relevance/maturity gating** enforced at the facilitation surface (t-2), and **guard-minimums** per scope via a fork-carried core guard-floor seam (t-3). Governance was **reuse, not reinvent** — the only new machinery is the policy data; guards/supervisor/approvals/audit are Sunrise-core, untouched. That completes the **facilitation critical-path spine** (`f-map → f-journey-state → f-engine → f-guidance → f-facilitation-agents → f-policies`). **`f-facilitation-agents` (13)** shipped just before it (#68 / #70; detailed plan: [[f-facilitation-agents]]) — the binding + surface, mechanism-only. **`f-module-bindings` (07)** + **`f-module-config` (06)** are **shipped** (Simon; #33/#35/#50/#53 and #56/#58), and unblocked **`f-ops-views` (15)** (now **claimed by Simon and in flight**, detailed plan: [[f-ops-views]] — the module admin + structural journey explorer). **`f-emergence` (18)** is now **claimed by John and in flight** (detailed plan: [[f-emergence]]) — the structure-change proposal pipeline (F17) + the picked-up escalation kind (F15, sequenced first) + evaluation wiring; it reads `f-policies`' `autoApprove` knob and delivers f-policies' deferred escalation. The four unclaimed, claimable features run in parallel: **`f-engagement` (08)**, **`f-map-editor` (14)**, **`f-overlays` (19)** (pgvector similarity + proactive guidance), and **`f-atlas` (16)** (the composition view). Shipped so far: `f-seams` · `f-bootstrap` · `f-module-core` · `f-map` · `f-slots` · `f-journey-state` · `f-engine` · `f-slot-capture` · `f-module-bindings` · `f-module-config` · `f-guidance` · `f-facilitation-agents` · `f-policies` (#73 / #74 / #75).
...
_Owner:_ TBD · _Status:_ **available to claim** ▲ (deps `f-module-bindings` + `f-facilitation-agents` + `f-slots` shipped ✅) · _Depends on:_ f-module-bindings, f-facilitation-agents, f-slots · _~3 PRs_
...
- **t-4** (deferred) — **escalation-pathway** kind (F15): `(signal, scope) → response` declarative wiring over the shipped bridges (`drainEngine` workflow dispatch + `notifyEscalation` + audit log). **Not built in this close-out** — the three shipped kinds cover the governance spine (F14) + both enforced gates (relevance F14, guard-minimums F16); F15 escalation is deferrable (no current deployment needs configured escalation, and it composes existing machinery). Tracked as an open follow-up: promote it as a thin t-4 PR when a deployment needs it, or fold it into **`f-emergence` (18)**, which touches the same approval/notify machinery. _f-policies is marked shipped for its core kinds; this is a conscious scope carve-out, not a silent drop._

### 18 · `f-emergence` — proposal pipeline + evaluation wiring

_Owner:_ John · _Status:_ **in flight** (deps `f-engine` + `f-facilitation-agents` shipped ✅) · _Depends on:_ f-engine, f-facilitation-agents · _~5 PRs_ · **detailed plan: [[f-emergence]]**

Emergence through one narrow gate (F17); the spine is never written raw. Also **picks up f-policies' deferred escalation kind (F15) as t-1** — it fits here (shared approval/notify machinery). Promoted-task table lives in [[f-emergence]].

- **t-1** — **Escalation-pathway kind (F15)** + a 2nd fork-carried core seam (a post-detection guard-event contributor). _Delivers f-policies' deferred t-4._
- **t-2** — `StructureChangeProposal` model + the **map-subject** pipeline (schema validation → `validateGraphInvariants` → risk-class → **own** approval state; `createdBy="agent:<slug>"`); v1 targets the map only (module-config/policy subjects are additive).
- **t-3** — Proposal admin API + approve/reject + the `autoApprove` reader (ships `none`).
- **t-4** — Evaluation wiring — named-metric scoring (+ post-hoc supervisor) over framework (`facilitation`/`module`) conversations, read from `AiMessage`.
- **t-5** — Judge seeds via a **new** `isSystem:false` framework agent-seed scaffold (#303) — the first framework agent seeds.
