...
| Lead                    | Simon Holmes                                                                                                                                                                                                                                      |
| Status                  | `in flight` — `f-seams` + `f-bootstrap` + `f-module-core` + `f-map` + `f-slots` **shipped**; `f-journey-state` **in flight** (John); **4 features available to claim** (`f-module-config`, `f-module-bindings`, `f-engagement`, `f-slot-capture`) |
...
| 09  | `f-journey-state`       | John            | **in flight**                                               | f-map ✅                                          | 3    | Journey state models + access discipline         |
...
**Claimable right now (▲) — four open features.** **`f-journey-state` (09)** — the head of the remaining critical path (it feeds `f-engine` → `f-guidance`) — is now **claimed by John and in flight** (detailed plan: [[f-journey-state]]). `f-slots` (05) is **shipped**, which unblocks **`f-slot-capture` (10)** — the `fill_slot` / `get_state` capabilities over the insert-only value engine `f-slots` landed. The rest of the unclaimed set — **`f-module-config` (06)**, **`f-module-bindings` (07)**, **`f-engagement` (08)** — extends the module layer and runs in parallel.
...
_Owner:_ John · _Status:_ **in flight** (dep `f-map` shipped) · _Depends on:_ f-map · _~3 PRs (2 promoted)_ · **detailed plan: [[f-journey-state]]**
