...
| Lead                    | Simon Holmes                                                                                                                                                                                                                                                                                                                                            |
| Status                  | `in flight` — `f-seams` + `f-bootstrap` + `f-module-core` + `f-map` + `f-slots` + `f-journey-state` + `f-engine` + `f-slot-capture` + `f-module-bindings` + `f-module-config` + `f-guidance` **shipped**; `f-facilitation-agents` (John) **in flight**; **4 features available to claim** (`f-engagement`, `f-map-editor`, `f-ops-views`, `f-overlays`) |
...
| 15  | `f-ops-views`           | _unclaimed_     | **available** ▲                                 | f-module-config ✅, f-module-bindings ✅, f-journey-state ✅ | 4    | Module admin + journey explorer                  |
...
_Owner:_ _unclaimed_ · _Status:_ **available to claim** ▲ (deps `f-module-config` + `f-module-bindings` + `f-journey-state` all shipped ✅) · _Depends on:_ f-module-config, f-module-bindings, f-journey-state · _~4 PRs_
