...
| Lead                    | Simon Holmes                                                                                                                                                                                                                                                                                                                                                                                                      |
| Status                  | `in flight` — `f-seams` + `f-bootstrap` + `f-module-core` + `f-map` + `f-slots` + `f-journey-state` + `f-engine` + `f-slot-capture` + `f-module-bindings` + `f-module-config` + `f-guidance` + `f-facilitation-agents` + `f-policies` **shipped**; `f-ops-views` (Simon) **in flight**; **4 features available to claim** (`f-engagement`, `f-map-editor`, `f-overlays`, `f-atlas`; `f-emergence` also unblocked) |
...
**Claimable right now (▲) — five open features.** **`f-policies` (17)** is now **shipped** (#73 / #74 / #75; detailed plan: [[f-policies]]) — the typed governance-policy kinds under one `FacilitationPolicy` table (a `kind` discriminator + per-kind Zod payload, F14): the spine + **auto-approval** stored knob (t-1), **relevance/maturity gating** enforced at the facilitation surface (t-2), and **guard-minimums** per scope via a fork-carried core guard-floor seam (t-3). Governance was **reuse, not reinvent** — the only new machinery is the policy data; guards/supervisor/approvals/audit are Sunrise-core, untouched. That completes the **facilitation critical-path spine** (`f-map → f-journey-state → f-engine → f-guidance → f-facilitation-agents → f-policies`). **`f-facilitation-agents` (13)** shipped just before it (#68 / #70; detailed plan: [[f-facilitation-agents]]) — the binding + surface, mechanism-only. **`f-module-bindings` (07)** + **`f-module-config` (06)** are **shipped** (Simon; #33/#35/#50/#53 and #56/#58), and unblocked **`f-ops-views` (15)** (now **claimed by Simon and in flight**, detailed plan: [[f-ops-views]] — the module admin + structural journey explorer). The five unclaimed, claimable features run in parallel: **`f-engagement` (08)**, **`f-map-editor` (14)**, **`f-overlays` (19)** (pgvector similarity + proactive guidance), **`f-atlas` (16)** (the composition view), and **`f-emergence` (18)** (the structure-change proposal pipeline — now unblocked, and the natural next step off John's just-shipped `f-policies`, whose `autoApprove` knob it reads). Shipped so far: `f-seams` · `f-bootstrap` · `f-module-core` · `f-map` · `f-slots` · `f-journey-state` · `f-engine` · `f-slot-capture` · `f-module-bindings` · `f-module-config` · `f-guidance` · `f-facilitation-agents` · `f-policies` (#73 / #74 / #75).
