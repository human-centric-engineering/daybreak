...
| Lead                    | Simon Holmes                                                                                                                                                                                                                                                                                          |
| Status                  | `in flight` — `f-seams` + `f-bootstrap` + `f-module-core` + `f-map` + `f-slots` + `f-journey-state` + `f-engine` **shipped**; `f-module-bindings` **in flight** (Simon), `f-slot-capture` **in flight** (John); **3 features available to claim** (`f-module-config`, `f-engagement`, `f-map-editor`) |
...
| 10  | `f-slot-capture`        | John            | **in flight**                                               | f-slots ✅                                             | 3    | `fill_slot` / `get_state` capture capabilities   |
| 11  | `f-engine`              | John            | **shipped** (#34 / #36 / #37 / #38)                         | f-map ✅, f-journey-state ✅                           | 5    | Deterministic engine + GraphStore                |
| 12  | `f-guidance`            | _unclaimed_     | blocked → f-slot-capture                                    | f-engine ✅, f-slot-capture                            | 5    | Guidance service, capabilities, chat injection   |
...
**Claimable right now (▲) — four open features.** **`f-engine` (11)** is **shipped** (#34 / #36 / #37 / #38) — the deterministic spine (GraphStore · availability · the sole-writer `applyEvent` · publish-invariants). It unblocks **`f-map-editor` (14)** (both deps `f-map` + `f-engine` now shipped) — the map editor + journey dry-run simulator — and clears the `f-engine` edge off **`f-guidance` (12)** (now blocked only on `f-slot-capture`) and **`f-emergence` (18)** (now blocked only on `f-facilitation-agents`). **`f-slot-capture` (10)** — the `fill_slot` / `get_state` capture capabilities over the insert-only value engine `f-slots` landed — is now **claimed by John and in flight** (detailed plan: [[f-slot-capture]]); it is the last blocker on **`f-guidance` (12)**, the head of the remaining critical path. **`f-module-bindings` (07)** is **claimed by Simon and in flight** (detailed plan: [[f-module-bindings]]) — it makes registered modules functional and unblocks `f-atlas` (16). The rest of the unclaimed set — **`f-module-config` (06)**, **`f-engagement` (08)**, and **`f-map-editor` (14)** (the engine-exercising map editor + dry-run simulator, newly unblocked) — runs in parallel.
...
_Owner:_ Simon Holmes · _Status:_ **in flight** (dep `f-module-core` shipped) · _Depends on:_ f-module-core · _~4 PRs_ · **detailed plan: [[f-module-bindings]]**
...
_Owner:_ John · _Status:_ **in flight** (dep `f-slots` shipped) · _Depends on:_ f-slots · _~3 PRs_ · **detailed plan: [[f-slot-capture]]**
...
_Owner:_ TBD · _Status:_ **blocked** → f-slot-capture (`f-engine` shipped ✅) · _Depends on:_ f-engine, f-slot-capture · _~5 PRs_
