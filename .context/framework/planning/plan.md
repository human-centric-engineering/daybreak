...
| Lead                    | Simon Holmes                                                                                                                                                                                                                                                   |
| Status                  | `in flight` — `f-seams` + `f-bootstrap` + `f-module-core` + `f-map` + `f-slots` + `f-journey-state` **shipped**; `f-engine` **in flight** (John); **4 features available to claim** (`f-module-config`, `f-module-bindings`, `f-engagement`, `f-slot-capture`) |
...
**Claimable right now (▲) — four open features.** **`f-engine` (11)** — the deterministic engine at the head of the remaining critical path (it feeds `f-guidance` → `f-facilitation-agents`), unblocked when `f-journey-state` (09) shipped — is now **claimed by John and in flight** (detailed plan: [[f-engine]]). `f-slots` (05) is **shipped**, which unblocks **`f-slot-capture` (10)** — the `fill_slot` / `get_state` capabilities over the insert-only value engine `f-slots` landed. The rest of the unclaimed set — **`f-module-config` (06)**, **`f-module-bindings` (07)**, **`f-engagement` (08)** — extends the module layer and runs in parallel.
