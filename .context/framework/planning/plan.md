...
| Lead                    | Simon Holmes                                                                                                                                                                                                                                                                     |
| Status                  | `in flight` — `f-seams` + `f-bootstrap` + `f-module-core` + `f-map` + `f-slots` + `f-journey-state` + `f-engine` **shipped**; `f-module-bindings` **in flight** (Simon); **4 features available to claim** (`f-module-config`, `f-engagement`, `f-slot-capture`, `f-map-editor`) |
...
**Claimable right now (▲) — four open features.** **`f-engine` (11)** is **shipped** (#34 / #36 / #37 / #38) — the deterministic spine (GraphStore · availability · the sole-writer `applyEvent` · publish-invariants). It unblocks **`f-map-editor` (14)** (both deps `f-map` + `f-engine` now shipped) — the map editor + journey dry-run simulator — and clears the `f-engine` edge off **`f-guidance` (12)** (now blocked only on `f-slot-capture`) and **`f-emergence` (18)** (now blocked only on `f-facilitation-agents`). `f-slots` (05) is **shipped**, which unblocks **`f-slot-capture` (10)** — the `fill_slot` / `get_state` capabilities over the insert-only value engine `f-slots` landed. **`f-module-bindings` (07)** is **claimed by Simon and in flight** (detailed plan: [[f-module-bindings]]) — it makes registered modules functional and unblocks `f-atlas` (16). The rest of the unclaimed set — **`f-module-config` (06)**, **`f-engagement` (08)** — extends the module layer and runs in parallel.
