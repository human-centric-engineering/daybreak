...
| Lead                    | Simon Holmes                                                                                                                                                                                                                                                                         |
| Status                  | `in flight` — `f-seams` + `f-bootstrap` + `f-module-core` + `f-map` + `f-slots` + `f-journey-state` **shipped**; `f-module-bindings` **in flight** (Simon), `f-engine` **in flight** (John); **3 features available to claim** (`f-module-config`, `f-engagement`, `f-slot-capture`) |
...
| 11  | `f-engine`              | John            | **in flight**                                               | f-map ✅, f-journey-state ✅                           | 5    | Deterministic engine + GraphStore                |
...
_Owner:_ John · _Status:_ **in flight** (deps `f-map` + `f-journey-state` shipped) · _Depends on:_ f-map, f-journey-state · _~5 PRs (4 promoted)_ · **detailed plan: [[f-engine]]**
