...
| Lead                    | Simon Holmes                                                                                                                                                                                                                                                                                             |
| Status                  | `in flight` — `f-seams` + `f-bootstrap` + `f-module-core` + `f-map` + `f-slots` + `f-journey-state` + `f-engine` + `f-slot-capture` + `f-module-bindings` **shipped**; `f-guidance` (John) + `f-module-config` (Simon) **in flight**; **2 features available to claim** (`f-engagement`, `f-map-editor`) |
...
| 06  | `f-module-config`       | Simon Holmes    | **in flight**                             | f-module-core ✅                                          | 4    | Generic Zod config form + config versioning      |
...
| 15  | `f-ops-views`           | _unclaimed_     | blocked → f-module-config                 | f-module-config, f-module-bindings ✅, f-journey-state ✅ | 4    | Module admin + journey explorer                  |
...
**Claimable right now (▲) — three open features.** **`f-guidance` (12)** — the critical-path head, unblocked when `f-slot-capture` (10) shipped — is now **claimed by John and in flight** (detailed plan: [[f-guidance]]): the advisory layer that ranks the engine's already-eligible options using the freshest slots (F12), the built-in guidance capability family agents consume, and the per-turn context injection that is the first moment the experience is _felt_ in a conversation. **`f-module-bindings` (07)** is **shipped** (Simon; #33 / #35 / #50 / #53; detailed plan: [[f-module-bindings]]) — a registered module is now functional: agents bind into seats, capabilities register namespaced, module events fire published workflows, and bound agents inherit the module's knowledge scope; f-guidance's surface-scoping task completes the enforcement half of its scope-refusal seam (a coordination point, not a dependency). **Unblocks `f-atlas` (16)** (whose only remaining blocker is f-facilitation-agents). **`f-module-config` (06)** is now **claimed by Simon and in flight** (detailed plan: [[f-module-config]]): the operator-config half of the module spine — config validated against each module's own Zod schema (A4), versioned via a point-in-time `ModuleVersion` snapshot table (A10), API-first with the client form deferred to `f-ops-views` (15). Shipping it **unblocks `f-ops-views` (15)**. The two remaining unclaimed, claimable features — **`f-engagement` (08)** and **`f-map-editor` (14)** (the engine-exercising map editor + dry-run simulator) — are mutually independent and run in parallel. Shipped so far: `f-seams` · `f-bootstrap` · `f-module-core` · `f-map` · `f-slots` · `f-journey-state` · `f-engine` (#34 / #36 / #37 / #38) · `f-slot-capture` (#42 / #43 / #44 / #45 / #46) · `f-module-bindings` (#33 / #35 / #50 / #53).
