...
| Lead                    | Simon Holmes                                                                                                                                                                                                                                                                                                                                                                               |
| Status                  | `in flight` — `f-seams` + `f-bootstrap` + `f-module-core` + `f-map` + `f-slots` + `f-journey-state` + `f-engine` + `f-slot-capture` + `f-module-bindings` + `f-module-config` + `f-guidance` + `f-facilitation-agents` **shipped**; `f-policies` (John) + `f-ops-views` (Simon) **in flight**; **4 features available to claim** (`f-engagement`, `f-map-editor`, `f-overlays`, `f-atlas`) |
...
| 17  | `f-policies`            | John            | **in flight**                                   | f-facilitation-agents ✅                                     | 4    | Typed facilitation policy kinds                  |
| 18  | `f-emergence`           | _unclaimed_     | **available** ▲                                 | f-engine ✅, f-facilitation-agents ✅                        | 4    | Structure-change proposal pipeline + eval wiring |
...
**Claimable right now (▲) — four open features.** **`f-facilitation-agents` (13)** is now **shipped** (#68 / #70; detailed plan: [[f-facilitation-agents]]) — the second scope of the one binding mechanism: `FacilitationAgentBinding` (flat `(agentId, role)` map, hand-FK cascade + `@@unique([role])`, validated against a framework-owned `FACILITATION_ROLES` constant) + the `withAdminAuth` binding API, and the facilitation **surface** chat route (`resolveFacilitationSurface(userId, role)` — role-keyed, `public`-visibility gated, no scope, since guidance caps are scope-agnostic). Shipped **mechanism-only**: a fork creates its own facilitation agents and binds them (no seeded family — the conditional seed task was dropped). It was the **last blocker on `f-atlas` (16)**, **`f-policies` (17)**, and **`f-emergence` (18)**: **`f-policies` (17) is now claimed by John and in flight** (detailed plan: [[f-policies]]) — the typed facilitation policy kinds (relevance/maturity gating, guard-minimums, escalation, `autoApprove:none`), where f-facilitation-agents' deferred per-scope guard settings land; **`f-atlas` (16)** and **`f-emergence` (18)** are now **available ▲**. **`f-module-bindings` (07)** + **`f-module-config` (06)** are **shipped** (Simon; #33/#35/#50/#53 and #56/#58) — a registered module is fully functional (seat bindings, namespaced capabilities, module-event workflows, knowledge scope) and operator-configurable (per-module Zod config, versioned via `ModuleVersion`); together they unblocked **`f-ops-views` (15)** (now **claimed by Simon and in flight**, detailed plan: [[f-ops-views]] — the module admin + structural journey explorer that mount 06's config form and 07's binding UIs). The four unclaimed, claimable features — **`f-engagement` (08)**, **`f-map-editor` (14)**, **`f-overlays` (19)** (pgvector similarity + proactive guidance), and **`f-atlas` (16)** (the composition view, now that f-facilitation-agents shipped) — are mutually independent and run in parallel (`f-emergence` (18) is also unblocked but sits closest to John's in-flight `f-policies`). Shipped so far: `f-seams` · `f-bootstrap` · `f-module-core` · `f-map` · `f-slots` · `f-journey-state` · `f-engine` · `f-slot-capture` · `f-module-bindings` · `f-module-config` · `f-guidance` · `f-facilitation-agents` (#68 / #70).
...
_Owner:_ John · _Status:_ **in flight** (dep f-facilitation-agents shipped ✅) · _Depends on:_ f-facilitation-agents ✅ · _~4 PRs_ · **detailed plan: [[f-policies]]**

Typed policy kinds under one table, not a rules blob (F14). Ships `autoApprove: none`. Directly downstream of f-facilitation-agents, which deferred its per-scope guard settings here.
...
_Owner:_ TBD · _Status:_ **available to claim** ▲ (deps `f-engine` + `f-facilitation-agents` shipped ✅) · _Depends on:_ f-engine, f-facilitation-agents · _~4 PRs_
