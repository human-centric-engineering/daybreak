...
| Lead                    | Simon Holmes                                                                                                                                                                                                                                                                                    |
| Status                  | `in flight` — `f-seams` + `f-bootstrap` + `f-module-core` + `f-map` + `f-slots` + `f-journey-state` + `f-engine` + `f-slot-capture` **shipped**; `f-module-bindings` **in flight** (Simon); **4 features available to claim** (`f-module-config`, `f-engagement`, `f-map-editor`, `f-guidance`) |
...
| 12  | `f-guidance`            | _unclaimed_     | **available** ▲                                             | f-engine ✅, f-slot-capture ✅                         | 5    | Guidance service, capabilities, chat injection   |
...
**Claimable right now (▲) — four open features.** **`f-slot-capture` (10)** is now **shipped** (#42 / #43 / #44 / #45 / #46) — the `fill_slot` / `get_state` capture capabilities over the insert-only value engine `f-slots`, plus the reusable framework capability-registration seam, sensitivity masking, #307 typed extraction, and per-agent read/write exposure. It was the **last blocker on `f-guidance` (12)** — so `f-guidance`, the head of the remaining critical path, is now **available ▲** (its other dep `f-engine` already shipped). **`f-engine` (11)** shipped earlier (#34 / #36 / #37 / #38) — the deterministic spine — and had already unblocked **`f-map-editor` (14)** (both deps `f-map` + `f-engine` shipped) and cleared the `f-engine` edge off `f-emergence` (18). **`f-module-bindings` (07)** is **claimed by Simon and in flight** (detailed plan: [[f-module-bindings]]) — it makes registered modules functional and unblocks `f-atlas` (16). The four unclaimed, claimable features — **`f-guidance` (12)** (newly unblocked; the critical-path head), **`f-module-config` (06)**, **`f-engagement` (08)**, and **`f-map-editor` (14)** — run in parallel.
...
_Owner:_ TBD · _Status:_ **available** ▲ (both deps shipped ✅) · _Depends on:_ f-engine ✅, f-slot-capture ✅ · _~5 PRs_
