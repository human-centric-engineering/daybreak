...
| ---- | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---- | -------- | --- |
| t-1  | **Framework admin scaffolding + nav seam + module list page**                                                        | `lib/framework/admin-nav.ts` (new), `lib/app/admin-nav.ts` (fill), `lib/app/leaf-admin-nav.ts` (new, empty), `lib/framework/admin/endpoints.ts` (new), `app/admin/framework/modules/{page,components}`, `tests/…`  | —    | **done** | #66 |
| t-2  | **Module detail shell (tabbed) + Config tab + Versions tab** (UI over 06)                                            | `app/admin/framework/modules/[slug]/{page,layout?}`, `components/admin/framework/module-detail/*` (tabbed shell, config-form renderer, versions list), `tests/…`                                                   | t-1  | **done** | #69 |
| t-3  | **Module lifecycle writes** — `PATCH` + `DELETE /modules/[slug]` + Settings tab + danger-zone delete                 | `lib/framework/modules/{service,api-schemas}.ts` (new write fns), `app/api/v1/admin/framework/modules/[slug]/route.ts` (PATCH/DELETE), `components/admin/framework/module-detail/settings-tab.tsx`, `tests/…`      | t-2  | **done** | #83 |
| t-4a | **Agents tab** (+ declared-seats `GET …/agent-roles`) (UI over 07)                                                   | `lib/framework/modules/bindings/queries.ts` (+`getModuleAgentRoles`), `app/api/v1/admin/framework/modules/[slug]/agent-roles/route.ts` (new), `components/admin/framework/module-detail/agents-tab.tsx`, `tests/…` | t-2  | **done** | #84 |
| t-4b | **Workflows tab** (UI over 07)                                                                                       | `components/admin/framework/module-detail/workflows-tab.tsx`, `tests/…`                                                                                                                                            | t-2  | **done** | #86 |
| t-4c | **Knowledge tab** (UI over 07) **+ extract the shared binding-tab primitives**                                       | `components/admin/framework/module-detail/{knowledge-tab,use-binding-roster,use-row-actions,row-confirm}.tsx`, refactor `{agents,workflows}-tab.tsx`, `tests/…`                                                    | t-2  | **done** | #89 |
| t-5a | **Journey read API** — gated `getJourneyById`/`listJourneys` + `admin-queries` enrichment + list/detail endpoints    | `lib/framework/facilitation/journey/{queries (+getJourneyById/listJourneys),admin-queries,api-schemas,view}.ts`, `app/api/v1/admin/framework/journeys/{route,[journeyId]/route}.ts` (new), `tests/…`               | t-1  | **done** | #94 |
| t-5b | **Explorer UI** — read-only `@xyflow` canvas (own node components + BFS journey mapper) + timeline replay + nav item | `app/admin/framework/journeys/{page,[journeyId]/page}.tsx`, `components/admin/framework/journey-explorer/*` (canvas+mapper+replay), `lib/framework/admin-nav.ts` (+Journeys), `tests/…`                            | t-5a | **done** | #97 |

**Nine promoted PRs** (t-1, t-2, t-3, t-4a, t-4b, t-4c shipped; **t-4 split into t-4a/t-4b/t-4c** and **t-5 split into t-5a/t-5b** at build — see the t-4 / t-5 detail + [[planning-retro#B22]]/[[planning-retro#B1]]; t-5a/t-5b remaining). The board's 4 bundled-in-08 analytics moved out (§decision 1); the split is drawn on the **UI-over-shipped-API vs builds-one-new-endpoint** seam and, within t-4, on each binding kind's distinct machinery, so each PR is one cohesive slice — and the security-sensitive `DELETE` + invalidation was isolated in t-3 for focused review, not buried in a binding-UI PR. **Dependency shape:** t-2 ships the tabbed detail **host** (the tab array); t-3 and each of t-4a/b/c **add a tab entry** to it (the host-first extension point — minimal churn, one array entry each). t-4a/b/c are independent of each other and of t-3, so after t-2 those can proceed in any order; t-4a landed first so t-4b/c mirror its reviewed binding-tab shape. **t-5a/t-5b** are the journey explorer's own API→UI pair (t-5b mounts on t-5a's contract), independent of the module-detail tabs.
...
**Split into three sub-tasks (t-4a / t-4b / t-4c), one per binding kind** — the binding views deferred from [[f-module-bindings]] (reconciliation #1). The split follows [[planning-retro#B22]] (size typed-kinds-under-one-host by each kind's _distinct enforcement machinery_, not reflexively one-per-kind): the three kinds have genuinely different machinery (Agents: a role picker + the one new read endpoint + primary-seat + config; Workflows: a free-form `eventType` + `inputTemplate` JSON + `enabled`; Knowledge: doc-XOR-tag grant/revoke over two rosters), and [[planning-retro#B1]] confirms none is a foldable sliver — each is a cohesive ~t-2/t-3-sized vertical slice appending one tab to t-2's host. Build **t-4a first** so the reusable "binding tab" component shape (stitched read table with degraded/tombstone rows + an inline create form whose picker roster loads on demand) is reviewed once and **t-4b/t-4c mirror it** ([[planning-retro#B21]] → clean reviews). All write endpoints (agents/workflows `POST` + `[bindingId]` `PATCH`/`DELETE`; knowledge `POST` + query-param `DELETE`) and all picker rosters (agents, workflows, knowledge documents, tags) already ship — so t-4 builds **no** write endpoints and **one** small read endpoint (t-4a's roles GET, below). Tier: pure framework-tier, no core seam, no upstream issue (verified at build, [[planning-retro#B17]]).

#### t-4a · Agents tab (+ the declared-seats read endpoint)

- **New read endpoint** `GET /modules/[slug]/agent-roles` → `{ registered, roles }` (folded in per [[planning-retro#B1]] — its only consumer is this tab): the module's declared `ModuleDefinition.agentRoles`, read from the code registry (`getModuleAgentRoles` in `bindings/queries.ts`), 404 on unknown module. The Agents tab needs it to offer a **role picker** instead of a blind free-text guess (the same list the bind service validates against). Backed by the parallel server fetch in the detail page.
- **Agents tab** (`agents-tab.tsx`) — list `GET …/agents` (stitched `agent{ name, slug, isActive, deletedAt }`; **Deleted** badge for a tombstoned agent, **Inactive** for a merely-deactivated one, a degraded "unknown agent" row when the agent is gone); **bind** (agent picker from the orchestration roster fetched on demand + role picker from `agent-roles` + `isPrimary`; `POST`), **make-primary** (`PATCH …/[id] { isPrimary:true }`), **unbind** (two-step confirm `DELETE`). The per-binding opaque `config` override has no operator-facing consumer yet → not edited here (deferred, noted).
- **Done when:** the tab lists bindings with the tombstone/inactive/degraded states and supports bind / make-primary / unbind; unregistered & no-seats notices; the roles query + endpoint + tab tested (mocked prisma/registry, mocked `apiClient`); **gates green** before opening the PR.

#### t-4b · Workflows tab

- List `GET …/workflows` (stitch `workflow{ name, hasPublishedVersion }`; flag "**won't fire yet**" when no published version); bind (workflow picker + free-form `eventType` text + `inputTemplate` JSON + `enabled`; `POST`), toggle-enabled/edit-template (`PATCH …/[id]`), remove (`DELETE`). Mirrors t-4a's component shape. No new endpoint (eventType is free-form X1).
- **Done when:** the tab lists + supports add/edit/remove with the won't-fire flag; component tests green; **gates green**.

#### t-4c · Knowledge tab (+ extract the shared binding-tab primitives)

- List `GET …/knowledge` → `{ documents, tags }`; grant a document **XOR** a tag (document/tag pickers from the orchestration rosters; `POST`), revoke (`DELETE ?documentId | ?tagId`). Two structurally-identical sections (Documents, Tags) render through one generic internal `GrantSection`. No new endpoint.
- **The rule-of-three extraction (deferred from t-4b's `/code-review`):** by t-4c the roster-fetch-with-in-flight-guard and the two-step-confirm/row-lock state machine were byte-shared by the agents + workflows tabs, and Knowledge is the third use — so extract the shared primitives and refactor **all three** tabs onto them: `use-binding-roster.ts` (`useBindingRoster<T>(url)` — lazy fetch, fetch-once in-flight guard, `capped`), `use-row-actions.ts` (`useRowActions()` — confirm/busy/error + `locked` + `run()`, carrying the corrected row-lock), `row-confirm.tsx` (`<RowConfirm>`). Designed against all three concrete cases (Knowledge has two rosters + a revoke, not one roster + an unbind — the primitives generalise, they don't assume a single roster/unbind). The redundant `!rosterError` guard normalizes away in the refactor.
- **Done when:** the tab lists documents + tags and supports grant/revoke; the three primitives are extracted and agents/workflows refactored onto them (their tests green unchanged — behaviour preserved); component + primitive tests green; **gates green**.
...
**Split at build into t-5a (read API) + t-5b (explorer UI)** on the **UI-over-shipped-API vs builds-one-new-endpoint** seam — the same seam that drew the t-3/t-4 split ([[planning-retro#B22]]) — because the plan-time "reuse `workflow-builder/` primitives in read-only mode" reuse turned out to be **light on the library, heavy on the feature**: the workflow node/edge types (`pattern-node.tsx`, `retry-edge.tsx`) are workflow-specific (render step config), and there is **no layout library** in the deps (only `@xyflow/react`), so the canvas needs its own journey node components + a BFS journey mapper (analogous to `workflow-mappers.autoLayout`) + a replay reducer. That is a cohesive UI slice distinct from the read API, which is itself a self-contained, security-relevant new-endpoint slice (CLAUDE.md API-first). Bundling would make one oversized PR mixing a new read API with a heavy canvas; the split keeps each PR one kind and gives the UI a reviewed, stable API contract to mount on. Neither is a foldable sliver ([[planning-retro#B1]]).

#### t-5a · Journey read API

- **Gated primitives** — add to the framework journey query module (`lib/framework/facilitation/journey/queries.ts`, next to `getJourney`/`getNodeStates`/`getJourneyTimeline`): `getJourneyById(viewer, journeyId, scope?)` (find-by-id, then `canRead(viewer, row.userId, scope)` → `ForbiddenError`; `null` if the row is absent) and `listJourneys(viewer, scope?, opts?)` (`subjectScope(viewer, scope)` → `where` fragment → `findMany` ordered `startedAt desc`, paginated). This **resolves the Journey-list open question** (below): a thin, subject-scope-ready gated list, not a compose-of-existing.
- **Admin enrichment** — a new `lib/framework/facilitation/journey/admin-queries.ts` composes the guarded primitives for the two admin surfaces, keeping `queries.ts` the pure guarded-primitive layer: the **list** stitches the map name (`FacilitationGraph` by `graphSlug`) + a batched node-state progress count (one `groupBy` over `UserNodeState` for the already-scoped journey ids — no N+1); the **detail bundle** composes `getJourneyById` → `getGraphDetail(graphSlug)` (published `definition` parsed via `mapDefinitionSchema` → `{ nodes, edges }`) → `getNodeStates` → `getJourneyTimeline` (full, chronological, for replay).
- **Endpoints** — `GET /api/v1/admin/framework/journeys` (list; `paginatedResponse`) and `GET /api/v1/admin/framework/journeys/[journeyId]` (enriched detail bundle). `withAdminAuth`; construct the viewer `{ userId: session.user.id, isAdminSupport: true }` **explicitly** (not `role === 'ADMIN'`); the queries gate-before-Prisma so cross-user reads are admin-support-only until #366/#367 land. Wire types (string dates) in a new `lib/framework/facilitation/journey/view.ts`; query/param schemas in `api-schemas.ts` (reuse `shared/route-params.ts` `parseCuidParam`).
- **Done when:** query unit tests (by-id gating/404, list subject-scoping, enrichment stitch/counts) + route contract tests (admin guard, explicit `isAdminSupport` viewer, 404/403, envelope) green; **gates green** before the PR. Pure framework-tier (confirm per [[planning-retro#B17]]).

#### t-5b · Explorer UI (read-only canvas + replay)

- **Explorer UI** — `app/admin/framework/journeys/page.tsx` (picker list over the t-5a list endpoint) → `app/admin/framework/journeys/[journeyId]/page.tsx` (detail) + a read-only `@xyflow/react` canvas (the raw `ReactFlow` + `Background`/`Controls`/`MiniMap` primitives, **own** journey node components + a **journey mapper** analogous to `workflow-mappers.ts` with a BFS layout over `prerequisite`/`unlocks` edges) colouring nodes by `UserNodeState` status (unvisited/available/active/visited/completed) over the published map structure.
- **Individual replay** — a pure client reducer over the t-5a timeline (`node_entered`/`node_completed` events, already written by `f-engine`) progressively revealing reached/completed nodes up to a scrubber index; subject-scope-ready (the same `canRead`/`scope` seam, so owner/cohort scoping is additive later).
- **Nav** — add a "Journeys" item to the Framework section in `lib/framework/admin-nav.ts` (currently "Modules" only).
...
- **Done when:** an operator can pick a user's journey, see it laid out on a read-only map coloured by node state, and replay their traversal from the event log; explorer component tests (mocked `apiClient`/`serverFetch`) + journey-mapper unit tests green; **gates green** before opening the PR.
...
- ~~**Journey list endpoint** — compose existing queries or add a small `listJourneys`?~~ **RESOLVED at t-5a build:** added a thin, subject-scope-ready `listJourneys(viewer, scope, opts)` gated primitive to `queries.ts` (plus a `getJourneyById` by-id gated read), with the admin-surface stitching/counting in a new `admin-queries.ts` so `queries.ts` stays the pure guarded-primitive layer. A compose-of-existing wasn't viable — there was no by-id gated read and no scoped enumerator; both are one-liners over `subjectScope`/`canRead`.
...
## Follow-ups (deferred, recorded — surfaced by t-4 build/review)

- **Searchable roster pickers** — the binding tabs' pickers (agents, workflows, knowledge documents/tags) fetch the roster endpoint's max page (`ROSTER_LIMIT = 100`) with no search, so a target past the cap is unreachable via the UI (flagged in-UI, but the only escape hatch is "deactivate unused"). Materially more limiting for **Knowledge** (document/tag corpora routinely exceed 100). Follow-up: switch `useBindingRoster` to a typeahead that passes `?q=` to the (already `q`-capable) list endpoints. Not blocking — recorded here so it isn't only a UI string. (t-4c `/code-review`, Altitude.)
- **`<BindingCreateForm>` shell** — after the t-4c primitive extraction (`useBindingRoster`, `useRowActions`, `RowConfirm`), the create-form _shell_ (the `roster.error ? alert : fields` branch + cap-hint + errors `<ul>` + submit/cancel pair) is still structurally repeated across the three tabs' forms. The subtle-logic duplication was the priority and is done; this markup shell is a lower-value, higher-divergence dedup (workflows' 2-row layout, per-tab cap nouns) deliberately deferred to keep the t-4c PR focused. Extract if it drifts. (t-4c `/code-review`, Reuse.)
- **Journey detail timeline is unbounded (windowed replay)** — `getJourneyDetailForAdmin` returns the journey's **full** event timeline (`getJourneyTimeline` with no `take`) because replay needs the whole ordered sequence; a naïve `limit` would silently truncate the replay to a partial traversal (worse than unbounded). Fine for the real single-user profile (events are bounded by map traversal), but a long-lived or repeatable-node-heavy journey could return a large payload. Follow-up: if that materialises, move replay to a **windowed** model (paged/segmented timeline the t-5b scrubber pages through) rather than a flat cap — a real scale change, not a one-line limit. Not blocking. (t-5a `/code-review`, Altitude.)
