...
**Seven promoted PRs** (t-1, t-2, t-3 shipped; **t-4 split into t-4a/t-4b/t-4c** at build, one per binding kind — see the t-4 detail + [[planning-retro#B22]]/[[planning-retro#B1]]; t-5 remaining). The board's 4 bundled-in-08 analytics moved out (§decision 1); the split is drawn on the **UI-over-shipped-API vs builds-one-new-endpoint** seam and, within t-4, on each binding kind's distinct machinery, so each PR is one cohesive slice — and the security-sensitive `DELETE` + invalidation was isolated in t-3 for focused review, not buried in a binding-UI PR. **Dependency shape:** t-2 ships the tabbed detail **host** (the tab array); t-3 and each of t-4a/b/c **add a tab entry** to it (the host-first extension point — minimal churn, one array entry each). t-4a/b/c and t-5 are independent of each other and of t-3, so after t-2 the remaining tasks can proceed in any order (or parallel); t-4a lands first so t-4b/c mirror its reviewed binding-tab shape.
