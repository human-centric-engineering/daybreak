...
| t-4c | **Knowledge tab** (UI over 07)                                                                       | `components/admin/framework/module-detail/knowledge-tab.tsx`, `tests/…`                                                                                                                                                                 | t-2  | **backlog**   | —   |
...
- List `GET …/knowledge` → `{ documents, tags }`; grant a document **XOR** a tag (document/tag pickers from the orchestration rosters; `POST`), revoke (`DELETE ?documentId | ?tagId`). Mirrors t-4a's shape (simplest — grant/revoke, no update). No new endpoint.
- **Done when:** the tab lists documents + tags and supports grant/revoke; component tests green; **gates green**.
