...
| t-4a | **Agents tab** (+ declared-seats `GET …/agent-roles`) (UI over 07)                                                   | `lib/framework/modules/bindings/queries.ts` (+`getModuleAgentRoles`), `app/api/v1/admin/framework/modules/[slug]/agent-roles/route.ts` (new), `components/admin/framework/module-detail/agents-tab.tsx`, `tests/…` | t-2  | **in flight** | —   |
| t-4b | **Workflows tab** (UI over 07)                                                                                       | `components/admin/framework/module-detail/workflows-tab.tsx`, `tests/…`                                                                                                                                            | t-2  | **in flight** | —   |
| t-4c | **Knowledge tab** (UI over 07) **+ extract the shared binding-tab primitives**                                       | `components/admin/framework/module-detail/{knowledge-tab,use-binding-roster,use-row-actions,row-confirm}.tsx`, refactor `{agents,workflows}-tab.tsx`, `tests/…`                                                    | t-2  | **in flight** | —   |
| t-5a | **Journey read API** — gated `getJourneyById`/`listJourneys` + `admin-queries` enrichment + list/detail endpoints    | `lib/framework/facilitation/journey/{queries (+getJourneyById/listJourneys),admin-queries,api-schemas,view}.ts`, `app/api/v1/admin/framework/journeys/{route,[journeyId]/route}.ts` (new), `tests/…`               | t-1  | **backlog**   | —   |
| t-5b | **Explorer UI** — read-only `@xyflow` canvas (own node components + BFS journey mapper) + timeline replay + nav item | `app/admin/framework/journeys/{page,[journeyId]/page}.tsx`, `components/admin/framework/journey-explorer/*` (canvas+mapper+replay), `lib/framework/admin-nav.ts` (+Journeys), `tests/…`                            | t-5a | **backlog**   | —   |
