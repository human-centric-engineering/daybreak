...
| 19  | Evaluation & Monitoring         | `evaluate`, `supervisor`                               | Score output against a rubric (1–10); threshold gates next step. `supervisor` audits the entire execution post-hoc with evidence-cited verdict | data-pipeline, code-review, provider-model-audit                 |
...
### `evaluate` vs `supervisor` — when to use which

Both belong to pattern 19. They differ on **scope** and **lineage**:

| Concern        | `evaluate`                                                | `supervisor`                                                                                                                                                            |
| -------------- | --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Scope          | One step's output                                         | Entire execution trace                                                                                                                                                  |
| Lineage        | Workflow's own model (`modelOverride`, else task default) | Independent judge model (`JUDGE_MODEL` env, falls through to defaults)                                                                                                  |
| Output         | `{ score, reasoning, passed }` — numeric on a scale       | `{ verdict, score, summary, strengths, weaknesses, anomalies, unverifiedAreas, confidence }` — structured JSON with evidence-cited citations                            |
| Anti-optimism  | None (workflow grades itself)                             | Post-hoc citation validator strips citations that don't ground in the trace; verdict downgrades when the `minWeaknesses` floor breaks; required `unverifiedAreas` array |
| Position       | Anywhere (mid-flow gates)                                 | Anywhere — but most useful as a terminal-position step, where it can audit the actual outcome                                                                           |
| Runtime toggle | None                                                      | `inputData.__runSupervisor: false` skips with `expectedSkip: true`                                                                                                      |
