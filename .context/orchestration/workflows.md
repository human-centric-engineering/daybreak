...
**What it ships:** canvas + pattern palette, single `PatternNode` custom type for all 19 step types (15 originals + `supervisor` + `report` + `chat_turn` + `judge_call`), per-step config editors, live debounced validation (this validator + FE-only extra checks), red-ring errors, and a save flow (create via details dialog → POST; edit via direct PATCH).
...
- **`expectedSkip` flag** — optional step-config boolean (default `false`). When `errorStrategy: 'skip'` fires on a step that has `expectedSkip: true`, the trace entry carries the flag through so the admin UI renders the row as muted "Optional step skipped: <reason>" rather than the standard skip styling, and the expanded view uses a slate "Skip reason" pane instead of a red "Error" pane. The diagnostic is preserved either way; the flag only tones down styling for steps you actively expect to skip in normal operation (optional enrichment, missing env vars, vendor downtime). Don't use it for skips that signal a broken integration. Wired through `stepErrorConfigSchema`, `StepResult.expectedSkip`, `ExecutionTraceEntry.expectedSkip`.
- **Human approval pause** — `human_approval` executor interpolates `prompt` against the live `ExecutionContext` (so `{{stepId.output}}` references expand to accumulated upstream outputs) then throws `PausedForApproval` carrying the interpolated text. Engine flips the row to `paused_for_approval` and exits cleanly. The admin UI renders the prompt as markdown — see [`admin/orchestration-approvals.md`](../admin/orchestration-approvals.md#expand-view).
...
| `chat_turn`         | Nothing (re-runs the LLM + re-persists)          | Re-driving a `chat_turn` step re-invokes the provider and writes a new pair of `AiMessage` rows — there's no per-step dedup gate today. Safe-to-retry on transient provider failures (each retry costs the same as a fresh first call); avoid putting it inside a `parallel` branch unless you want both branches to produce + persist separate replies. Cost-cap enforcement still applies via the engine's outer loop. |
...
`llm_call`, `chain`, `parallel`, `route`, `guard`, `evaluate`, `judge_call`, `plan`, `rag_retrieve`, `human_approval` are not in the table because they do not have external side effects — `llm_call` and `judge_call` re-run (paying the token cost), the rest are pure dispatch logic over already-completed steps.

### `chat_turn` — conversational step with auto-loaded history

The `chat_turn` step is the workflow-side analogue of the streaming chat handler. It closes the parity gap where `llm_call` is single-shot (no history) and `agent_call` only does within-step multi-turn (no cross-run memory).

Config (`chatTurnConfigSchema`):

- `agentSlug` (string, required) — which agent's system prompt + provider + model to use
- `conversationId` (string, required) — template-interpolated; for inbound-trigger flows the canonical value is `{{trigger.conversationId}}`
- `message` (string, required) — the new user message; template-interpolated (e.g. `{{trigger.text}}`)
- `historyLimit` (number, default 20) — how many prior `AiMessage` rows to load (oldest-first; user + assistant roles only)
- `persistMessages` (boolean, default true) — whether to write the new user + assistant turns to `AiMessage`. Set `false` for preview / what-if runs that shouldn't pollute the conversation log
- `temperature` / `maxTokens` / `modelOverride` / `reasoningEffort` — step-level overrides that beat the agent's defaults

Behaviour:

1. Loads up to `historyLimit` prior `AiMessage` rows for the conversation, oldest-first.
2. Composes the agent's system prompt via the same `resolveEffectivePrompt` + `composeSystemPromptString` path the streaming chat handler uses (persona / voice / guardrails inheritance is byte-identical).
3. Builds `[system, ...history, user]` and calls `provider.chat()`.
4. Persists both new turns to `AiMessage` in one transaction with provenance pins (`agentVersionId`, `workflowExecutionId`, `providerSlug`, `modelId`).
5. Logs cost under `CostOperation.CHAT` with `metadata.source: 'chat_turn'` + `metadata.historyTurnsLoaded`.
6. Persistence failure does NOT block the step output — logged + swallowed so a partial write can't break the workflow; the next run's history will be one turn shorter but the workflow still completes.

v1 scope (deliberately): no tool calls (chain a `tool_call` step after, or use `agent_call` if you need mid-turn capability dispatch), no streaming (workflows are batch), no citations (chain `rag_retrieve` before and interpolate). All additive in v2 without breaking the public contract.

The `tpl-inbound-conversation-handler` template ([source](../../prisma/seeds/data/templates/inbound-conversation-handler.ts)) is the canonical two-step shape: `chat_turn` → `tool_call(send_message_to_channel)`. Covered end-to-end by [`tests/integration/orchestration/inbound-chat-turn-loop.test.ts`](../../tests/integration/orchestration/inbound-chat-turn-loop.test.ts) which asserts the cross-execution memory loop with a stateful in-memory Prisma mock.
...
## Reasoning effort on step configs

Every step config that already exposes `temperature` / `maxTokens` also accepts an optional `reasoningEffort` (`'minimal' | 'low' | 'medium' | 'high' | null`). The runtime forwards it into `LlmOptions.reasoningEffort` when the executor calls `runLlmCall`. See [`llm-providers.md` § Reasoning-effort knob](./llm-providers.md) for the per-provider wire mapping.

| Step type      | What it controls                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `llm`          | The single LLM call this step issues                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `judge_call`   | The judge agent's LLM call (forwarded via `agent.reasoningEffort` since the judge is a real `AiAgent`; the step doesn't override).                                                                                                                                                                                                                                                                                                                                                                           |
...
| `orchestrator` | The **planner LLM only**. Delegated agents continue to use their own `AiAgent.reasoningEffort`. By design — see `orchestratorConfigSchema`. The trace row's headline `model` / `provider` / `requestParams` reflects the planner too, not whichever delegation ran last: the orchestrator tags planner entries `source: 'planner'` and post-tags delegation entries `source: 'delegation'`, and `rollupTelemetry()` prefers the last planner entry when any are present. Token totals still sum across both. |
