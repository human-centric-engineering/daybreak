...
**Error codes returned in `CapabilityResult.error.code`:** `invalid_args`, `invalid_binding` (returned fail-closed when the per-agent `customConfig` JSON fails its Zod schema, when `forcedUrl` resolves to a non-URL, or when a `${env:VAR}` reference points at an unset env var; admin must repair the row before the agent can call this capability again), `url_not_allowed`, `host_not_allowed`, `auth_failed`, `rate_limited`, `http_error`, `timeout`, `response_too_large`, `request_aborted`, `request_failed`.

**Env-var templating.** `forcedUrl` and `forcedHeaders` values may contain `${env:VAR_NAME}` references — resolved on every call against `process.env`. Use this when the value itself is a credential (a Slack incoming-webhook URL pinned via `forcedUrl`, a literal `Authorization` line on `forcedHeaders`) and you want it to live in env vars only. The literal template stays in `customConfig`; the secret only exists in process memory at request time. Missing env var → `invalid_binding`, no outbound request, no silent downgrade. Save-time hint: the binding API returns `meta.warnings.missingEnvVars` listing referenced vars not currently set in the running process. See [external-calls.md → Env-var templating](./external-calls.md#env-var-templating-in-stringy-fields) for the full spec.

**Multipart/form-data.** The capability accepts an optional `multipart` arg — `{ files: [{ name, filename?, contentType, data: base64 }], fields?: Record<string,string> }` — mutually exclusive with `body`. Used for endpoints that require named file parts (Gotenberg HTML→PDF being canonical). Per-file cap is 8 MB base64; total request body cap is 25 MB. HMAC auth is rejected with multipart bodies (multipart can't be HMAC-signed deterministically). See [external-calls.md → Multipart bodies](./external-calls.md#multipartform-data-bodies) for the spec.
