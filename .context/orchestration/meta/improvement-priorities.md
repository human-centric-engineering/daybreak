...
**Last updated:** 2026-05-28 (item 42 (Capability emergency-disable / quarantine) marked ✅ Done after this PR — three additive columns on `AiCapability` + dispatcher state check between registry lookup and per-agent binding + two POST sub-routes (quarantine/unquarantine) + `capability.quarantined` / `capability.unquarantined` hook events + four admin surfaces (detail-page card, agent banner, list badge + header-chip filter, dashboard panel) + 21 new test cases. System capabilities are intentionally quarantine-able; auto-expiry is read-time only; documented in a new "Quarantine" section in `.context/orchestration/capabilities.md`). — 2026-05-27 (item 24 (WhatsApp / SMS channel) marked ✅ Done after PR #218 — inbound Twilio + WhatsApp Cloud adapters, symmetric outbound adapter framework with two adapters, `send_message_to_channel` capability, `(channel, fromAddress)` conversation key with concurrent-inbound serialisation, inbound-conversation-handler starter template, admin trigger CRUD UI, and the `sms-whatsapp-inbound-reply.md` recipe; Tier 6 sequencing re-ordered to **28 → 25 → 27 → 26**). — 2026-05-27 (recent-PR sweep — Phases 1–4 of dataset-driven evaluations (PRs #237, #246, #250, #259, #260) are now ✅ Done as a stack: registered grader registry with polymorphic agent/workflow subject, empirical cost estimator that walks the workflow DAG, three Ragas-style RAG judge agents seeded, **two new workflow step types** (`judge_call`, `chat_turn`) bringing the registry to 19, the `workflow_as_judge` and `pairwise_judge_agent` graders, pairwise verdicts on the experiment compare page, the eval CI gate with API-key auth on eval-run endpoints, dataset cold-start "Generate from description" + post-commit editable cases + trajectory diagnostics, and the dev in-process maintenance ticker. **Knowledge-retrieval mode** on `AiAgent` (`model` / `first_turn` / `every_turn` / `keywords` + `knowledgeTriggerKeywords`, PR #260) closes the "agent ignores its KB" gap that prior model-decided-only retrieval left open. **Multi-tenancy seam** (PR #258 — `TENANCY_MODE` env + `lib/db/client.ts` chokepoint + runnable RLS spike + `.context/architecture/multi-tenancy.md` playbook) — single-tenant remains the default with zero baked-in tenancy machinery. Retention purge already noted at item 14 — extended in PR #257 to cover executions / evaluations / MCP audit. — 2026-05-20 (item 47 (Conversation provenance bundle) marked ✅ Done after PR #196; item 40 (Stuck-execution / live-engine admin surface) re-graded Low–Moderate after PR #202's running-step side table; item 35 (Shadow / canary version routing) re-graded Moderate after PR #197 + PR #198; item 23a re-anchored on item 47's shipped provenance scalars; item 49 prerequisite chain shortened to #46; Tier 8 sequence dropped #47. — 2026-05-16 (consistency pass — every pending item carries `**Priority justification.**` + `**Difficulty.**`; Tier 8 + Tier 7 added; Tier 5 gained item 23a))
...
| 2a  | Workflow-step provenance (engine analogue of item 2)             | High      | Low–Moderate | ✅ Done (this PR)                 |
| 2b  | Provider matrix taxonomy split (tierRole vs deploymentProfiles)  | High      | Moderate     | ✅ Done (this PR)                 |
...
### 2a. Workflow-step provenance — ✅ Done

**Why it mattered.** Item 2 closes the chat-handler half of "where did this claim come from": chat assistant turns ground tool-result text in citation envelopes the user can verify. The workflow engine's other half was open — an LLM/agent step inside a workflow produces structured JSON ("propose tier change from `embedding` to `thinking`") and an admin reviewing it has no signal whether the claim came from training, a web search the workflow injected, or a knowledge-base lookup. The provider-model-audit workflow surfaced this concretely: it kept proposing that **Qwen2.5-72B is an embedding engine** because the LLM was free to confabulate against training knowledge, present it as `confidence: 'high'`, and label the resulting `reason` cell identically to a Brave-grounded claim. The structured approval UI rendered both as the same readonly text. Admins had no scannable way to spot un-grounded proposals.

**What shipped (this PR).** Workflow-step analogue of item 2: a `ProvenanceItem` contract (`source` + `confidence` + optional `reference` / `snippet` / `stepId` / `note`) that LLM steps emit as `output.sources`. The engine lifts the array onto `ExecutionTraceEntry.provenance` (typed field, schema-validated, silently dropped on shape failure so provenance is non-load-bearing). A new `display: 'sources'` variant on the structured approval UI's `ReviewSchema` renders colour-coded pills (`web · qwenlm.github.io ●●●`, `training · ●○○`, `kb · doc.pdf ●●●`) with hover tooltips showing the full reference, snippet, and note. The trace viewer mirrors the same pill component in a Sources sub-panel under the Input/Output grid. A reusable `provenanceRequiredRule()` helper returns a guard-prompt fragment authors paste into their `guard.config.rules` to enforce attribution on selected top-level array fields, using the existing 2-retry budget. The audit workflow opts in: the three producer prompts (`analyse_chat`, `analyse_embedding`, `discover_new_models`) now render Brave search results as a numbered block and require every change, new-model, and deactivation proposal to attribute via the contract — `training_knowledge` confidence is capped at `medium`/`low`, and the embedding analyser gains an explicit "general-purpose LLMs are out-of-scope" scope check naming Qwen2.5-72B as the prototype mis-route.

**Why this is workflow-step provenance, not item 47.** Item 47 (Conversation provenance bundle) pins per-message agent/workflow/model versions plus chunk refs onto `AiMessage` rows — that's per-conversation audit defensibility for chat surfaces. This is per-claim attribution inside a workflow execution's step output. They share vocabulary and complement each other on a deployment that uses both chat and workflows; neither is a substitute for the other.

**Critical files (for reference):** `lib/orchestration/provenance/types.ts` (contract + engine extractor), `lib/orchestration/provenance/guard-rules.ts` (opt-in rule fragment), `lib/orchestration/engine/orchestration-engine.ts` (sequential + parallel trace push lift), `prisma/seeds/data/templates/provider-model-audit.ts` (first adopter), `lib/orchestration/review-schema/types.ts` (`display: 'sources'`), `components/admin/orchestration/approvals/sources-field.tsx` (pill renderer), `components/admin/orchestration/workflow-builder/execution-trace-entry.tsx` (trace viewer panel), `.context/orchestration/provenance.md` (canonical doc).

### 2b. Provider matrix taxonomy split — ✅ Done

**Why it mattered.** Item 2a shipped the provenance primitive so admins could see _where_ a workflow's proposals came from. The audit workflow's first real run after that showed a deeper problem: the `tierRole` enum was packing four orthogonal dimensions (capability depth, role-in-deployment, deployment locus, modality) into one column. Qwen2.5-72B is legitimately a thinking-tier chat LLM that is also sovereign-deployable; the enum forced a single choice. The audit workflow's LLM, seeing a chat-capable model classified as `local_sovereign`, kept proposing tier changes that argued generic framings ("this is a chat model, not an embedding model") against unrelated current values. No amount of prompt tightening could fix a schema that couldn't represent the truth.

**What shipped (this PR).** Split deployment locus out of `tierRole` into a separate `deploymentProfiles String[]` array on `AiProviderModel`. Narrowed `tierRole` to the five actual capability tiers (`thinking`, `worker`, `infrastructure`, `control_plane`, `embedding`). Non-destructive Prisma migration (`20260516120000_add_deployment_profiles`) adds the column and re-classifies `local_sovereign` rows to `tierRole='worker' + deploymentProfiles=['sovereign']` (Llama 3.x) or `tierRole='thinking' + deploymentProfiles=['sovereign']` (Qwen 2.5 72B). Zod schemas in `lib/validations/orchestration.ts` narrow `tierRoleSchema` and add `deploymentProfilesSchema`. The selector heuristic (`provider-selector.ts`) keeps tier-based scoring for non-private intents and gets a sovereign-aware scoring branch for the `private` intent. Audit workflow producers evaluate the two axes as separate questions with explicit "rationale must engage with currentValue" enforcement via a new Rule 10 in the validator guard. UI gets a Deployment field on the provider-model form. Maturity-analysis remains accurate; the change is a structural correction, not a feature add.

**Why this is workflow-design, not just data-modelling.** The Qwen-as-embedding miscategorisation was downstream of a schema that couldn't carry the right answer. Item 2a (provenance) made the failure visible; item 2b makes it preventable. Future workflows that mutate provider-model rows inherit a sharper taxonomy and a guard rule (`rationale-engages-currentValue`) that catches the framing-drift class of error generically.

**Critical files (for reference):** `prisma/schema/` (`deploymentProfiles String[] @default(["hosted"])`), `prisma/migrations/20260516120000_add_deployment_profiles/`, `types/orchestration.ts` (`TIER_ROLES`, `DEPLOYMENT_PROFILES`, `DEPLOYMENT_PROFILE_META`), `lib/orchestration/model-audit/enums.ts`, `lib/validations/orchestration.ts` (`tierRoleSchema`, `deploymentProfilesSchema`), `lib/orchestration/llm/model-heuristics.ts` (`deriveDeploymentProfiles`), `lib/orchestration/llm/provider-selector.ts` (sovereign-aware private branch), `lib/orchestration/llm/db-model-adapter.ts` (override priority), `prisma/seeds/009-provider-models.ts` (three reclassified rows), `prisma/seeds/data/templates/provider-model-audit.ts` (two-axis prompts + Rule 10), `components/admin/orchestration/provider-model-form.tsx` (new Deployment field), `.context/orchestration/meta/architectural-decisions.md` §3.11 (canonical rationale).
...
### 6a. Neutral supervisor — post-hoc workflow execution audit — ✅ Done

**Why it mattered.** Item 6 added per-turn judge scoring for _agent chat_. There was no equivalent for _workflow executions_ — the final step of complex workflows like provider-model-audit is an agent-written narrative ("Compile a consolidated audit report"). That agent shares model lineage with the proposal-generating steps, so it's marking its own homework. A workflow could retry five times, ship a degraded answer, and the recipient would only see the optimistic summary.

**What shipped.**

- **New `supervisor` step type.** Registered alongside the other 15. Runs after the workflow's main body, reads the entire `ctx.stepOutputs` map + workflow input/output, and emits a structured verdict: `verdict` (pass / concerns / fail / inconclusive), `score` (0..1), `summary`, `strengths[]` and `weaknesses[]` (every entry cites a specific `stepId` and a verbatim `evidenceQuote`), `anomalies[]`, `unverifiedAreas[]`, `confidence`.
- **Anti-optimism mechanics, structural not aspirational.** (1) Independent judge model via `JUDGE_MODEL` env (shared resolver with item 6 — `lib/orchestration/evaluations/judge-model.ts`). (2) Post-hoc citation validator walks every citation against the trace; citations whose stepId doesn't exist or whose quote isn't a verbatim substring of the cited step's output are stripped into `invalidCitations[]`. (3) `minWeaknesses` floor — when stripping breaks the floor, verdict downgrades (pass → concerns → fail). (4) Required `unverifiedAreas[]` makes blind spots first-class. (5) Calibration anchors in the prompt include an explicit _wrong-pass_ anti-example (a retry storm that still ends with output is NOT a pass). (6) Temperature 0.2; one retry at temperature 0 on parse failure; double-failure produces `inconclusive` verdict (never throws — operator still gets a signal).
- **Run-time toggle.** Reserved input key `inputData.__runSupervisor: boolean`. The "Run workflow" dialog (and the bespoke "Audit Models" dialog) renders a "Run neutral supervisor review" checkbox visible only when the workflow contains a `supervisor` step; unchecking sets the key false and the executor short-circuits with `expectedSkip: true` (no judge-model tokens billed).
- **Persistence.** Four nullable columns on `AiWorkflowExecution`: `supervisorVerdict`, `supervisorScore`, `supervisorReport`, `supervisorReviewedAt`. Written via a new generic `StepResult.contextPatch` mechanism — `mergeStepResult` accumulates patches into `ctx.pendingContextPatch`; `checkpoint()` and `finalize()` drain it into the data block of the next lease-guarded Prisma update. Allowlisted (`CONTEXT_PATCH_ALLOWLIST`) so a malformed patch can't reach engine-internal columns. Any future post-hoc audit step type can reuse the same write path without engine changes.
- **Retroactive review.** `POST /api/v1/admin/orchestration/executions/:id/review` runs the same anti-optimism stack over a terminal execution's persisted trace — for workflows whose template doesn't include the step or runs where the operator skipped at trigger time. Re-entry archives the prior verdict into `supervisorReport.previousVerdicts[]` with `triggeredBy: 'in_workflow' | 'retroactive'`.
- **UI.** Sixth summary card on `ExecutionDetailView` (verdict badge + score) with a `FieldHelp` popover explaining the four verdicts. "Review this execution" / "Re-review" button next to Cancel / Approve / Retry on terminal executions.
- **Shared core.** `lib/orchestration/supervisor/index.ts` factors the prompt scaffolding, schema, citation validator, and `runSupervisorAssessment` orchestrator. Both the executor (`runLlmCall` shim) and the endpoint (direct `getProvider` + `logCost`) call into it — provider-agnostic via an `LlmCallShim` callback.
- **First adopter.** Provider-model-audit template inserts `supervisor_review` between `compile_report` and `notify_complete`. Notification body leads with the supervisor's verdict before the optimistic report narrative.

**Critical files:** `lib/orchestration/supervisor/index.ts`, `lib/orchestration/engine/executors/supervisor.ts`, `lib/orchestration/evaluations/judge-model.ts`, `app/api/v1/admin/orchestration/executions/[id]/review/route.ts`, `prisma/migrations/20260517100000_supervisor_verdict_columns/`, `prisma/seeds/data/templates/provider-model-audit.ts`, `components/admin/orchestration/execution-detail-view.tsx`, `components/admin/orchestration/audit-models-dialog.tsx`, `components/admin/orchestration/workflow-builder/execution-input-dialog.tsx`.

**Tradeoffs surfaced in the work.**

- Judge can fabricate citations under structured-output pressure. The post-hoc validator strips them; when strip rate breaks the `minWeaknesses` floor the verdict downgrades. Multi-rater consensus (two LLM passes with verdict comparison) is the natural next escalation — left out of v1 to avoid doubling cost; reconsider if validator strip rate stays high in production.
- Truncation defaults to head + middle + tail sampling at 4KB per step. Operators with very long step outputs can opt into `includeStepOutputs: 'all'` (no truncation) when they accept the token bill, or `'terminal-only'` (full output for the last step; 1KB head for earlier ones).
- Supervisor verdict is advisory by default (`failOnVerdict: 'never'`). Authors can opt into `failOnVerdict: 'fail'` to terminate the workflow on a fail verdict — useful before irreversible apply steps, but provider-model-audit runs supervisor _after_ its capability dispatches so the verdict can audit the actual outcome.
- Aggregate dashboard ("which workflows fail review most often", verdict drift over time) is **out of scope** for this PR — it's the natural next consumer of the new columns and aligns with item 22.
...
- **Critical files**: `lib/orchestration/inbound/{types,registry,bootstrap,index}.ts`, `lib/orchestration/inbound/adapters/{generic-hmac,postmark,slack}.ts`, `app/api/v1/inbound/[channel]/[slug]/route.ts`, `lib/orchestration/scheduling/scheduler.ts` (drainEngine export), `lib/security/rate-limit.ts` (inboundLimiter), `prisma/schema/` + `prisma/migrations/20260507120000_add_inbound_triggers/`, `.context/orchestration/inbound-triggers.md` (canonical doc).
...
Concretely, the work is four reportable signals on a new admin "Feedback health" dashboard:
...
4. **Supervisor verdict mix** (item 6a) — per workflow, the percentage of recent executions where the supervisor returned `pass`, `concerns`, `fail`, or `inconclusive`. A rising `concerns` / `fail` rate is the strongest possible cross-step quality signal because the supervisor reads the entire trace, not one signal in isolation. `inconclusive` rising is itself a flag — the judge model is failing to parse.

The raw data already exists. `reflect` emits per-iteration events; `AiEvaluationLog` stores per-response scores; retry attempts are captured in the execution trace's `retries[]` array; supervisor verdicts live on `AiWorkflowExecution` columns (item 6a). The work is the aggregation query, the dashboard, and the threshold-cross alerting hook (reuse the existing webhook hook dispatcher — emit `feedback.health.degraded` events that fork operators can route to Slack or PagerDuty).
...
| #   | Improvement                                               | Value         | Effort        | Status            |
| --- | --------------------------------------------------------- | ------------- | ------------- | ----------------- |
| 24  | WhatsApp / SMS channel (Twilio + WhatsApp Cloud adapters) | Very high     | Moderate      | ✅ Done (PR #218) |
...
### 24. WhatsApp / SMS channel (Twilio + WhatsApp Cloud adapters) — ✅ Done (PR #218)

**Why it mattered.** The proactive-outreach, customer-experience, mutual-aid, and tenant-rights subcategories in `business-applications.md` describe audiences that overwhelmingly live in WhatsApp and SMS, not on partner websites. The embed widget structurally cannot reach them — it requires the user to find the right page on the right partner site and start typing. WhatsApp and SMS invert that: a known user is reached in the channel they already check.

**What shipped (PR #218).**

- **Two inbound adapters.** `TwilioAdapter` (SMS + Twilio-routed WhatsApp) verifies `X-Twilio-Signature` (HMAC over reconstructed URL + sorted POST params); normalises form-encoded bodies and `MediaUrlN` fields into the standard `NormalisedTriggerPayload` shape. `WhatsAppCloudAdapter` (Meta-direct WhatsApp Business) verifies `X-Hub-Signature-256`, handles the GET verification handshake, and normalises the nested webhook shape. Both register through `bootstrapInboundAdapters()` from env, mirroring the existing Slack / Postmark / generic-HMAC pattern.
- **Outbound adapter framework + two adapters.** A symmetric `lib/orchestration/outbound/` module with an `OutboundAdapter` interface, registry, and env-driven bootstrap. `TwilioOutboundAdapter` and `WhatsAppCloudOutboundAdapter` send via the existing HTTP fetcher; `AiOutboundMessage` ledger row writes every send with `channel`, `provider`, `externalId`, status, and cost attribution. New `channel` / `provider` columns on `AiConversation` recorded by the inbound resolver.
- **`send_message_to_channel` capability.** Resolves the target channel from the conversation's recorded inbound origin, so a single workflow replies on whichever channel the user reached us on — SMS, WhatsApp, and email all first-class without per-channel branching. Per-agent binding requires a `customConfig.allowedChannels[]` whitelist (fail-closed).
- **Conversation resolver + cross-execution memory.** `(channel, fromAddress)` key threads inbound turns through one persistent `AiConversation` row across executions, so SMS / WhatsApp conversations have the same memory continuity as embed chat. Concurrent inbound serialises onto a single conversation row (fix in `1221da72`) so two near-simultaneous messages cannot create a duplicate.
- **Inbound conversation handler template.** A new starter workflow template (`inbound-conversation-handler`) wires an inbound trigger → `chat_turn` → `send_message_to_channel`. The New Workflow page pre-fills from this template when arriving from the Inbound Triggers page; the New Trigger page shows a worked WhatsApp example end-to-end.
- **Admin UI.** Inbound Triggers list / create / edit pages under `/admin/orchestration/triggers`; conversation channel + provider display on the conversation list; empty-state copy that explains what a trigger actually is rather than the previous opaque framing.
- **Docs + recipe.** `.context/orchestration/recipes/sms-whatsapp-inbound-reply.md` walks through inbound triage → reply for both SMS and WhatsApp. Channel docs in `.context/orchestration/inbound-triggers.md` and `outbound-channels.md` cover the 24-hour WhatsApp window, Meta template approval, and Twilio media fetch flow.

**Critical files (for reference):** `lib/orchestration/inbound/adapters/{twilio,whatsapp-cloud}.ts`, `lib/orchestration/outbound/{types,bootstrap,adapters/{twilio,whatsapp-cloud}}.ts`, `lib/orchestration/inbound/{phone,conversation-resolver,url-reconstruct}.ts`, `lib/orchestration/capabilities/built-in/send-message-to-channel.ts`, `lib/orchestration/admin/inbound-trigger-starter.ts`, `prisma/migrations/` (channel/provider columns on `AiConversation`, `AiOutboundMessage` ledger), `app/admin/orchestration/triggers/`, `.context/orchestration/recipes/sms-whatsapp-inbound-reply.md`.

**Tradeoffs surfaced in the work.**

- **24-hour WhatsApp window is partner-managed, not platform-enforced.** Outbound to a WhatsApp number outside the active session window must use a pre-approved Meta template. The platform does not currently track session-window expiry per conversation — partners get a Meta error back at send time. Surfacing this in the outbound ledger as a structured error code is a follow-on.
- **No per-channel cost-dashboard split yet.** Twilio / Meta spend lands in `AiCostLog` keyed under the outbound provider name; the cost dashboard sums it with LLM cost. A per-channel breakdown is the natural extension and pairs with item 33 (per-end-user cost attribution).
- **Worked example calls out the multi-turn-context gap.** The WhatsApp recipe documents that conversation memory works across turns within an execution; cross-execution context is via the `(channel, fromAddress)` key on `AiConversation`. Documented honestly (commit `a0d205ce`) rather than papered over.
...
- **Items 24 and 25 (WhatsApp / SMS + email threading)** unlock user populations the embed widget cannot reach. Both extend the existing inbound-triggers framework rather than introducing new architecture, so the per-item effort is bounded. **Item 24 ✅ shipped in PR #218**, adding a symmetric outbound adapter framework on top of inbound — that framework is the natural carrier for item 25's `reply_in_email_thread` capability.
...
- **Item 28 (live handover)** is the operational trust loop — the explicit admission that not every conversation should end inside the agent. It is the most common pilot-blocker in regulated verticals and reuses the most existing primitives (hooks, approval queue UI patterns, `widgetConfig`). Now that item 24 has shipped, handover can route to Slack / SMS / WhatsApp duty channels via `send_message_to_channel` without per-pilot custom code.

Sequenced for shortest path to commercial impact (item 24 done): **28 → 25 → 27 → 26**. Item 28 next because it removes the most common pilot-blocker and item 24 supplies the duty-channel plumbing; items 25 and 27 picked up in the order partner conversations demand; item 26 last because it is the most architecturally ambitious and the natural "save for a slot with a clear partner ask" item.
...
| 42  | Capability emergency-disable / quarantine                      | Moderate      | Low–Moderate | ✅ Done (this PR) |
...
### 42. Capability emergency-disable / quarantine — ✅ Done (this PR)

**Why it mattered.** When an external capability misbehaves at scale — returning malformed data, hanging, rate-limit-exceeded across all calling agents — the admin needs a one-click "stop using this capability across every agent" action, not an "edit each agent's bindings one at a time" tour. The closest action pre-PR was to deactivate the global `AiCapability.isActive` flag, which was a hard delete — every agent lost the capability silently, the bindings persisted but no longer resolved, and there was no clear audit signal that "we are in degraded mode."

**What shipped (this PR).**

- **Schema.** Three additive columns on `AiCapability` — `quarantineState` (`active` / `quarantined-soft` / `quarantined-hard`, defaulting to `active`), `quarantineReason String?`, `quarantineUntil DateTime?`. Index on `quarantineState`. Migration (`20260527214250_add_capability_quarantine`) is hand-written to strip Prisma's auto-generated drift cleanup (the `searchVector` / `idx_message_embedding` drops and the `ai_conversation_inbound_key` rename) per the documented pattern in the schema comments.
- **Dispatcher guard.** A new state check between the registry lookup and the per-agent binding check (`lib/orchestration/capabilities/dispatcher.ts`, between steps 3 and 4 of `dispatch()`). Soft mode returns `{ code: 'capability_quarantined', skipFollowup: false, metadata: { mode, reason } }` with a "temporarily unavailable: <reason>" message the agent can route around. Hard mode returns the same code with `skipFollowup: true` and a firm message that does not include the reason — stops the model's tool loop without giving it ammunition to retry. Auto-expiry is read-time: a past `quarantineUntil` is treated as `active` without mutating the row. Resolution lives in a single exported helper (`resolveQuarantineState`) reused by the agent-detail page and the dashboard.
- **Hook events.** `capability.quarantined` and `capability.unquarantined` registered in `HOOK_EVENT_TYPES`. Payloads include capability id/slug/name, mode (quarantine only), reason, expiresAt (quarantine only), `previousMode` (unquarantine only), actor user id, ISO timestamp. Auto-expiry intentionally does **not** fire an unquarantined event — the stored state is unchanged.
- **POST sub-routes.** Two new endpoints under `/api/v1/admin/orchestration/capabilities/[id]/`: `quarantine` (body: `{ mode, reason, expiresAt? }`, rejects past expiry, validates via `quarantineCapabilitySchema`) and `unquarantine` (empty body, idempotent — no-op when already active with no audit row and no hook). Both bypass the `isSystem` guard that the routine `PATCH .../isActive=false` path enforces — emergency response must be able to disable any tool. Each writes a distinct audit row (`capability.quarantine` / `capability.unquarantine`) so post-incident review can find the response window without sifting through routine edits, and clears the dispatcher cache so the change takes effect on the next call.
- **Capability detail page.** New `<CapabilityQuarantineCard>` mounted above `<CapabilityForm>`. Two-view UI: active state shows mode selector / reason textarea / optional `datetime-local` for auto-lift + a Quarantine button that opens a confirmation `<AlertDialog>` naming up to 8 affected agents before submission; quarantined state shows the current mode badge, reason, optional auto-lift timestamp, and a Lift button. Plain-English `FieldHelp` popovers on every input.
- **Agent detail page.** New `<QuarantinedCapabilitiesBanner>` server component above the evaluation chart. Lists every bound capability currently in effective quarantine (auto-expiry honoured), with mode chip, reason, and link to the capability detail page. Hidden when there are none.
- **Capabilities list.** Per-row amber/destructive badge inline with the existing System badge. Amber "{N} quarantined" header chip next to the category filter when N > 0, toggleable to filter the table to quarantined rows. Both backed by the same inline `isCurrentlyQuarantined()` helper that mirrors `resolveQuarantineState`.
- **Dashboard.** New `<ActiveQuarantinesPanel>` between `<BudgetAlertsBanner>` and the summary stats. Lists every currently-quarantined capability with mode badge, reason, auto-lift time, and a deep link. Hidden when empty (absence is the all-clear signal). Mitigates "quarantine forgetting" by surfacing aged quarantines prominently.
- **Tests.** Six new dispatcher cases (soft / hard / expired auto-clears / future blocks / active proceeds / corrupt-value fails open), eight route cases (quarantine + system caps + past expiry + missing reason + 404 + unquarantine clear + idempotent + 404), four component cases for `QuarantineCard` (form gating, dialog opens with affected agents, POST payload shape, lift POSTs), three each for the banner and dashboard panel (empty hides, mode badges + reason + linking). Full orchestration suite stays at 4536 passing.
- **Docs.** New "Quarantine (incident disable)" section in `.context/orchestration/capabilities.md` covering the two modes, the dispatcher payload, auto-expiry semantics, system-capability quarantinability, routes, hook payloads, and admin surfaces. The pipeline numbering gains a step 3a for the quarantine gate.

**Critical files (for reference):** `prisma/schema.prisma` (`AiCapability` quarantine columns), `prisma/migrations/20260527214250_add_capability_quarantine/`, `lib/validations/orchestration.ts` (`quarantineCapabilitySchema`, `QuarantineState`), `lib/orchestration/capabilities/types.ts` (`QuarantineState`, `CapabilityRegistryEntry` extension), `lib/orchestration/capabilities/dispatcher.ts` (quarantine guard + `resolveQuarantineState`), `lib/orchestration/hooks/types.ts` (event registration), `app/api/v1/admin/orchestration/capabilities/[id]/quarantine/route.ts`, `app/api/v1/admin/orchestration/capabilities/[id]/unquarantine/route.ts`, `components/admin/orchestration/capability-quarantine-card.tsx`, `components/admin/orchestration/agents/quarantined-capabilities-banner.tsx`, `components/admin/orchestration/active-quarantines-panel.tsx`, `components/admin/orchestration/capabilities-table.tsx` (per-row badge + header chip + filter), `app/admin/orchestration/page.tsx` (panel mount + Prisma fetch), `app/admin/orchestration/agents/[id]/page.tsx` (banner mount + Prisma fetch), `app/admin/orchestration/capabilities/[id]/page.tsx` (card mount), `.context/orchestration/capabilities.md` (Quarantine section).

**Tradeoffs surfaced in the work.**

- **No `quarantinedAt` column.** The card used to show a "Quarantined N minutes ago" header but `AiCapability` has no `updatedAt`/`createdAt`. Adding the column for one display string didn't earn its keep — the audit log carries the timestamp for anyone who needs to reconstruct the incident timeline. Future addition is purely additive.
- **No background tick to clear expired quarantines.** Auto-expiry is honoured at read time only. The stored fields persist after the timestamp passes, which is correct for audit but means the capabilities-list filter shows zero "quarantined" rows while the audit log still has the (now-historical) state. Documented inline.
- **`window.location.reload()` on submit.** The card forces a full reload after quarantine / unquarantine rather than threading new state up to the server component. Simpler, single-screen-flicker tradeoff; if the page grows expensive a future refactor can replace with router refresh.
