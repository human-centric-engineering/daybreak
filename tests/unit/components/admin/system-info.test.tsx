...
// DAYBREAK — the framework tier's version, the middle row of the card's three.
const DAYBREAK_VERSION_FIXTURE = '0.4.2';
...
function statsFixture(overrides: Partial<SystemStats['system']> = {}): SystemStats {
  return {
    ...
    system: {
      ...
      daybreakVersion: DAYBREAK_VERSION_FIXTURE,
...
describe('SystemInfo', () => {
  ...
  it('renders the Daybreak framework version as a third, distinct tier', () => {
    // DAYBREAK — three tiers, three owners, three numbers that move independently
    // (.context/framework/VERSIONING.md). The card is where an operator answers
    // "what is this box running?", and answering it for two of the three tiers
    // leaves the question half-open. Asserting distinctness is the point: the
    // failure this guards is a row wired to the wrong constant, which renders a
    // plausible number under the right label and looks correct.
    render(<SystemInfo stats={statsFixture()} />);

    expect(screen.getByText('Daybreak framework')).toBeInTheDocument();
    expect(screen.getByText(DAYBREAK_VERSION_FIXTURE)).toBeInTheDocument();
    expect(DAYBREAK_VERSION_FIXTURE).not.toBe(SUNRISE_VERSION_FIXTURE);
    expect(DAYBREAK_VERSION_FIXTURE).not.toBe(APP_VERSION_FIXTURE);
  });
