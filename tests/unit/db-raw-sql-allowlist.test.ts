...
const ALLOWLIST: ReadonlyArray<{ file: string; calls: number; why: string }> = [
  ...
  // DAYBREAK — the framework tier's three sites, every one on a `framework_*`
  ...
  // Sunrise #799; ledger row in .context/framework/upstream-asks.md.
  {
    ...
    calls: 1,
    why: 'jsonb `||` merge on framework_user_node_state.progress, bound as a cast text parameter — one row, keyed on (journeyId, nodeKey)',
  },
  {
    file: 'lib/framework/facilitation/overlays/embed-sync.ts',
    calls: 1,
    why: 'pgvector upsert into framework_node_embedding (`$4::vector`) — map-scoped content, not subject data',
  },
  {
