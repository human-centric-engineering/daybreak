...
const ALLOWED_CALL_SITES: readonly string[] = [
  ...
  // default model, not on any configured agent, so writing a synthetic label
  // into `agentId` is exactly the #599/#600 bug — omitted instead. `actorUserId`
  // needs no guard: both callers hand it a real `User.id` (the admin session on
  // the supervise route; `ctx.userId` — itself a User FK on the execution — or
