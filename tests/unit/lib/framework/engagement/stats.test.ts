/**
 * Unit tests: module engagement stats (f-engagement t-3 · dwell added in f-engagement-analytics
 * t-2). Prisma is mocked; asserts each metric is derived from the stream, the ratings summary
 * (count / average / distribution / recent comments) is folded correctly, a malformed feedback
 * payload is skipped (not counted, no crash), the empty case yields zeros + null average, the
 * subject-scope `userId` filter threads into every query's WHERE, and dwell is sessionised
 * (entered→completed pairing within the gap, median, consume-once, empty→null).
 *
...
vi.mock('@/lib/db/client', () => ({
  prisma: { journeyEvent: { findMany: findMock } },
}));
...
/**
 * Every read is now a `findMany`; dispatch by args so each stream (distinct users, entered,
 * completed, feedback) can be armed independently. `entries`/`completions`/`returningUsers`
 * derive from the entered/completed rows, so those rows drive their assertions.
 */
...
beforeEach(() => {
  vi.clearAllMocks();
  // Sensible defaults; individual tests override. Entered rows: u1×3, u2×1, u3×2 → 6 entries,
  // 2 returning (u1, u3). Completed rows: 4 progression events.
  armStreams({
    distinctUsers: [{ userId: 'u1' }, { userId: 'u2' }, { userId: 'u3' }],
    entered: [
      { userId: 'u1', occurredAt: d('2026-07-01T10:00:00Z') },
      { userId: 'u1', occurredAt: d('2026-07-02T10:00:00Z') },
      { userId: 'u1', occurredAt: d('2026-07-03T10:00:00Z') },
      { userId: 'u2', occurredAt: d('2026-07-01T10:00:00Z') },
      { userId: 'u3', occurredAt: d('2026-07-01T10:00:00Z') },
      { userId: 'u3', occurredAt: d('2026-07-02T10:00:00Z') },
    ],
    completed: [
      { userId: 'u1', occurredAt: d('2026-07-01T10:05:00Z') },
      { userId: 'u1', occurredAt: d('2026-07-02T10:05:00Z') },
      { userId: 'u3', occurredAt: d('2026-07-01T10:05:00Z') },
      { userId: 'u3', occurredAt: d('2026-07-02T10:05:00Z') },
    ],
  });
});
...
describe('getModuleStats', () => {
  it('derives unique users / entries / completions / returning users from the stream', async () => {
    ...
    expect(stats.uniqueUsers).toBe(3); // distinct userId rows (any event)
    expect(stats.entries).toBe(6); // module.entered rows
    expect(stats.completions).toBe(4); // node_completed rows
    expect(stats.returningUsers).toBe(2); // u1 (3) and u3 (2) entered >1; u2 (1) did not
  });

  it('summarises feedback: count, 2dp average, full distribution, recent comments (newest first)', async () => {
    armStreams({
      distinctUsers: [{ userId: 'u1' }],
      feedback: [
